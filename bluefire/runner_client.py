...
class SubprocessRustRunner:
        ...
        *,
        ...
        try:
            ...
            if watchdog is not None and not self._process_exited_without_reap(watchdog):
                ...
                elif os.name == "nt" and not self._windows_containment.finish(watchdog):
                    raise RunnerTransportError(
            ...
            try:
                ...
                if watchdog is not None and self._process_exited_without_reap(watchdog):
                    containment_released = (
                        self._windows_containment.finish(watchdog)
                        if os.name == "nt"
    ...
    def _await_watchdog(
        ...
        *,
        ...
        while True:
            if self._process_exited_without_reap(process):
                if os.name == "nt" and not self._windows_containment.finish(process):
    ...
    def _spawn(
        ...
        *,
        ...
        process: subprocess.Popen[bytes] | None = None
        if os.name == "nt":
            system_directory = self._windows_containment.system_directory()
            ...
            windows_job = self._windows_containment.create_job()
        else:
            if self._kill_child_on_job_close:
                try:
        ...
        try:
            ...
            if windows_suspended:
                try:
                    self._windows_containment.resume_suspended(process)
                except RunnerTransportError:
                    if not self._windows_containment.terminate(process):
                        raise RunnerTransportError(
            ...
            if windows_job is not None:
                self._windows_containment.close_handle(windows_job)
    ...
    def _monitor_process(
        ...
        *,
        ...
        while True:
            if self._process_exited_without_reap(process):
                released = (
                    self._windows_containment.finish(process)
                    if os.name == "nt"
    ...
    def _terminate_process_tree(self, process: subprocess.Popen[bytes]) -> bool:
        if os.name == "nt":
            return self._windows_containment.terminate(process)
