...
| Malicious plugin/action package | Plugin metadata remains non-executable. Signed action packages are canonical, bounded, content-addressed, locally trusted, and signature-verified; execution is limited to reviewed compiled runner actions or digest-bound no-host-import WASM providers with fixed ABI exports, declared limits, and exact activation/inventory binding. Scripts, Python entry points, arbitrary host commands, native shared libraries, and imported host functions are rejected. | Publisher trust is local policy, not a public PKI. Review source, license, digest, build chain, limits, and publisher-key enrollment before activation. |
