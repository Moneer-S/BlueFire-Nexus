...
Known non-permission mismatches, including path and nested-field predicates, exclude unrelated records before permission availability is interpreted. Records that could still match retain missing-permission gaps and invalid-fact refusal. Every internal report keeps the sorted, deduplicated base fields from its selection even when evidence gaps or execution refusal prevent matching; suffix operators do not erase those requirements from exports.

A valid unavailable-permission record still states `effective_access: not_evaluated`. A selector contradicting that known literal excludes the record without creating a permission gap. This does not calculate effective access or make an availability selector usable without observed permission facts; invalid permission groups still refuse evaluation.
