...
Parsing uses an exact `{}` body for `internal`, and `{"source":"..."}` for Sigma, SQLite, YARA, and SPL. Internal uses the maintained structured matcher. Sigma parses exactly one rule with pySigma and converts it through the exact pinned SQLite backend; native SQLite accepts exactly one bounded `SELECT` from the fixed `logs` table and requires `fixture_id` once in the result. YARA uses YARA-Python with includes disabled and warnings treated as errors. YARA-L has no authoritative local evaluator and remains unavailable rather than being treated as YARA. SPL has only a structural checker: success records the bounded source and checker result but deliberately remains `hypothesis`. Missing authoritative adapters return `503` without advancing state; parser/compiler/query rejection is persisted as `rejected`.

Malicious and benign fixture actions accept explicitly named JSON fixtures, at most 128 and 1 MiB in total. YARA fixtures contain only `fixture_id` and text `data`; matching compiles the persisted source and uses a timeout. Internal uses the bounded structured matcher. Sigma is freshly reconverted and both Sigma and native SQLite execute in a query-only, authorizer-guarded, capped in-memory database; the response retains query identity, versions, mapped/unsupported fields, evaluated/matched IDs, result shape, and limits. Benign evaluation also requires a bounded `notes` list:
