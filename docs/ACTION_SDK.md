...
| `sandbox.execution.process-tree-cancellation-witness.v1` | empty object | Windows-only fixed child-mode witness for request-bound cooperative acknowledgement and complete Job-contained process-tree termination |
...
| `sandbox.peer.handoff.v1` | `port` (1024..65535) | Credential-bound movement of exactly one `staged/bundle.json` or `staged/bundle.jsonl` to a distinct one-shot memory-only receiver process on literal `127.0.0.1`; output retains opaque handles plus bounded process/lifecycle evidence |
...
The create, transform, exact-file discovery, collection, export, and peer-handoff descriptors are implementation version `2.0.0`; cleanup is `1.1.0`. The native-canary, process-tree cancellation witness, identity-seed, identity-inspection, and observability-variant representative descriptors are version `1.0.0` and declare `ready` in the compiled inventory. The other built-in descriptors remain `1.0.0`. Inventory consumers must validate those versions and readiness instead of inferring them from the stable `.v1` action IDs.
...
Native canary performs only deterministic compiled computation for 1..4096 rounds; it cannot name a process, command, script, executable, or effect. The Windows-only process-tree cancellation witness accepts no parameters and can launch only the packaged runner's fixed private child mode; its request-bound nonce, cooperative acknowledgement, watchdog lease, and Job containment establish that cancellation terminates the complete tree without exposing a generic process interface. Identity seed writes an exact 189-byte public synthetic JSON canary at `identity-material/public-canary.json`, and identity inspection reads only that bound artifact. Inspection returns its byte count, field count, and SHA-256 digest without returning values or enumerating siblings. These actions do not access or represent credentials.
