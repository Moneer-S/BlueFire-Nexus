...
| #122 | docs: refresh README + status snapshot + Loop F batch entries (test count 2094 → 2127) | `3c04bb6` | Pure docs PR. README test count badge bump and chain-narrative bullet; next_roadmap.md gains a Loop F entry; release_readiness.md §8.4 records #118-#121. |
| #123 | feat(scenarios): extend narrative-name bar across the catalog + polish FIN7 | `c56a869` | Loop F follow-up — every shipped scenario now satisfies the chain-narrative invariant pinned for the flagship (`name` ≥12 chars, contains a space, distinct from `step_id`). FIN7 step names tightened from terse labels (`C2 Beacon`) to chain-narrative phrasing (`Loader beacons HTTPS to attacker-controlled C2`); FIN7 `objective:` rewritten as a multi-line literal block calling out the simulate-only contract. New parameterized test `test_every_step_has_a_narrative_name` runs the bar over every shipped scenario. |
| #124 | docs: bump test count badge 2127 → 2137 + record PR #123 | `c94aa01` | Pure docs PR. README badge + status snapshot test count: 2127 → 2137 (matches +10 parametrized cases in `test_every_step_has_a_narrative_name`). |
| #125 | docs(changelog): record Loop F batch under [Unreleased] | `a5d3ddf` | CHANGELOG `[Unreleased]` block records the 8-PR Loop F batch (#118-#125) under Added / Changed / Tests so the next rc cut has a complete draft ready. |
| #126 | feat(scenarios): drop "Simulate"/"simulation" voice from apt29 + healthcare step names | `ff433f8` | Loop F voice-consistency follow-up. apt29 + healthcare step names dropped the redundant `Simulate`/`simulation` prefix/suffix, matching the flagship + FIN7 voice. After this PR every shipped standard scenario reads with consistent voice; the simulate-only contract is asserted at YAML/runtime level only, not redundantly inside step titles. |
| #127 | feat(viewer): timeline carries per-step severity badge | `d1fe8cd` | Audit lens "higher-risk steps should look higher-risk." The dashboard timeline now shows a `severity` column inline so a SOC analyst sees the chain's risk arc (low → medium → high → critical) without cross-referencing the risk-summary card. New `_step_severity_index` helper composes `"<module>:<step_id>"` keys to match the risk scorer. Steps without a risk match render an em-dash, not a fake "unknown" badge. |
| #128 | docs: bump test count badge 2137 → 2140 + record PR #126/#127 | `f67f204` | Pure docs PR. README badge bump and chain-narrative bullet extended to mention timeline severity column; release_readiness.md §8.4 records #124-#127. |
| #129 | feat(risk): rationale carries defender-facing matters_because line | `08ff50f` | Audit lens "risk rationale should mention why the step matters." New `_TACTIC_MATTERS_BECAUSE` map surfaces alongside `tactic_base=<tactic>` as `matters_because=<short text>` (e.g. `destructive endgame`, `data leaves perimeter`, `enables lateral expansion`). Chain-position language a reader without MITRE ATT&CK fluency understands. Both standard and tactic_pack legacy paths emit the line; out-of-tree modules get no synthesised reason. |
| #130 | docs: bump test count badge 2140 → 2143 + record PR #128/#129 | `c6bb7c3` | Pure docs PR. README badge bump and chain-narrative bullet extended to mention `matters_because` rationale line. |
| #131 | feat(reporting): surface per-step rationale in report.md "Why" line | `2c46d11` | Closes the surface-consistency gap left after #129: the dashboard's "Why" column and CLI's risk-summary "Why" column already rendered the rationale; `report.md` now also surfaces it via a `- Why: ...` line per module so the markdown report tells the same chain-position story as the dashboard. |
| #132 | docs: refresh ARCHITECTURE.md artifact list + bump badge to 2144 | `b076ea9` | Pure docs PR. ARCHITECTURE.md "Local-first artifact flow" section was missing `manifest.json` and `index.html` from the artifact list; now describes both, references chain-narrative fields (`run.scenario_objective`, propagation `narrative`, severity column, rationale `matters_because`), and notes the top-level `output/index.html` aggregator. README badge bump 2143 → 2144. |
| #133 | docs(changelog): record Loop F extension batch (#126-#131) | `674476c` | Pure docs PR. CHANGELOG `[Unreleased]` block extended with #126 (voice consistency), #127 (timeline severity column), #129 (`matters_because` rationale), #131 (`report.md` "Why" line) under Added with audit-lens framing. |
| #134 | docs(usage): describe new dashboard features in 'Monitoring a run' | `e78d9be` | Pure docs PR. USAGE_GUIDELINES.md "Monitoring a run" section now enumerates every dashboard section the viewer renders (header objective paragraphs, severity column, propagation narrative, rationale list); manifest + report.md descriptions extended with new fields. |
| #135 | docs(readme): list dashboard's chain-narrative features inline | `5d2aaaf` | Pure docs PR. README's main dashboard description (line 110) extended to surface scenario `objective:` block, per-step severity column, propagation narrative column, and rationale (tactic_base + matters_because) — closes the gap left after the Loop F feature work. |
| #136 | feat(cli,manifest): surface highest severity tier in CLI summary + result dict | `409ede9` | Closes a real CLI / dashboard consistency gap. New public `highest_risk_tier` helper exposes the same severity tier the dashboard header badge renders. Orchestrator's result dict now carries `highest_severity`; CLI summary table renders it as a `Highest severity` row right below `Scenario`. Operators running from CLI see whether their run ended at low or critical without opening the dashboard. |
