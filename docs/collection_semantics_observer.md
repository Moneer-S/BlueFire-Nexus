...
schedules it after `sandbox.collection.records.v1`,
`sandbox.collection.archive.v1` or `sandbox.collection.atomic-gzip.v1`, using the exact output declared by the approved
...
deadline, and byte limits remain in effect. The reader accepts JSONL, the
reviewed deterministic USTAR representation with exactly one regular member
(`fixtures/transformed.jsonl`), or one bounded gzip member with no stored name or
timestamp. Gzip checksum and decoded length must match. It does not extract files
or follow archive links, and rejects concatenated members, trailing bytes,
excessive expansion, extended headers, truncation and unsupported representations.
...
this condition detects retained synthetic values in any supported container:
