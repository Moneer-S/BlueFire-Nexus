...
it("renders the unchanged current Python projection with permission and failure facts", async () => {
  // Deterministic producer-to-component software evidence, not an installed or live journey.
  const source = producerContract.source_step;
  const evidence = producerContract.evidence[0]!;
  const retainedProjection = JSON.stringify(producerContract.projection);
  const value: RunRecord = {
    run_id: producerContract.run_id, mode: "execute", status: "completed",
    steps: [{ ...source, planner_decision_id: "authored-display-decision" }],
    evidence: { records: producerContract.evidence },
    ai_proposals: [{
      schema_version: "bluefire.ai-proposal-record.v4", run_id: producerContract.run_id,
      current_step_id: source.step_id, deterministic_decision_id: "authored-display-decision",
      outcome: source.status, provider_called: false,
      planner_state: { observations: producerContract.projection },
    }],
  };
  ...
  expect(within(groupRow).getByText("Enabled", { selector: "dd" })).toBeVisible();
  expect(within(otherRow).getByText("Not enabled", { selector: "dd" })).toBeVisible();
  expect(within(permissions).getByText("Effective access not evaluated.")).toBeVisible();
  const failureRow = within(region).getByText("Failure classification", { selector: "dt" }).parentElement!;
  expect(within(failureRow).getByText("Execution timeout", { selector: "dd" })).toBeVisible();
  expect(within(permissions).getByText("From run position 1")).toBeVisible();
  const observation = within(permissions).getByText("Observation 1", { selector: "strong" }).closest("li")!;
  await user.click(within(observation).getByText("Evidence reference", { selector: "summary" }));
  expect(within(observation).getByText(evidence.evidence_id, { selector: "code" })).toBeVisible();
  expect(within(observation).getByText(source.step_id, { selector: "code" })).toBeVisible();
  expect(JSON.stringify(producerContract.projection)).toBe(retainedProjection);
});
