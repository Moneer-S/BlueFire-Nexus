...
test("production operator UI completes authoring, management, run, replay, and compare", async ({ page }) => {
  ...
  const firstEvidence = page.locator(".evidence-records").getByRole("article").first();
  await expect(firstEvidence).toBeVisible();
  await expect(firstEvidence.locator("header .badge")).toHaveText("Synthetic");
  await firstEvidence.getByText("Evidence identity and provenance", { exact: true }).click();
  await expect(firstEvidence.locator(".evidence-meta")).toBeVisible();
  await expect(firstEvidence.locator(".evidence-meta")).toContainText("Producer");
  await page.getByText(/^Inspect detection candidates/).click();
  const detectionReview = page.locator("details").filter({ has: page.locator("summary", { hasText: /^Inspect detection candidates/ }) });
  const firstDetection = detectionReview.getByRole("article").first();
  await expect(firstDetection).toBeVisible();
  await expect(firstDetection.locator("header .badge")).toHaveText(/^(Hypothesis|Parsed|Fixture exercised|Observed exercised|Benign evaluated|Rejected)$/);
