...
function requiredEnvironment(name: string): string {
  ...
  if (!value) throw new Error(`Gate09 browser configuration is missing ${name}.`);
  return value;
}
...
function productionLaunchUrl(): URL {
  ...
  try {
    ...
    throw new Error("Gate09 requires one valid production loopback launch URL.");
  }
  if (
    ...
    throw new Error("Gate09 requires an HTTP loopback URL with one capability fragment.");
  }
  return parsed;
}
...
if (process.env.VITE_DEMO_MODE === "true") {
  throw new Error("Gate09 production browser evidence cannot run with demo mode enabled.");
}

export default defineConfig({
  testDir: "./tests/e2e",
  testMatch: "source-intake-production.spec.ts",
  timeout: 90_000,
