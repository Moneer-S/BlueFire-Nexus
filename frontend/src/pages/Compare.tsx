import { objectiveLabel, runLabel } from "../lib/run-presentation";
...
const EVERY_PLATFORM = ["linux", "macos", "windows"];
// A contract-compatible method can still be unrunnable here: the policy engine refuses a
// step whose action, profile and requested platform do not intersect. Name that restriction
// in the picker so the operator sees it before dispatch instead of after a refused run.
export function platformRestriction(platforms: string[] | undefined): string {
  const supported = (platforms ?? []).filter((item) => EVERY_PLATFORM.includes(item));
  if (!supported.length || supported.length === EVERY_PLATFORM.length) return "";
  return ` · ${supported.map((item) => item === "macos" ? "macOS" : item.charAt(0).toUpperCase() + item.slice(1)).join(" / ")} only`;
}
...
function ComparisonWorkspace() {
  ...
  const replayChoiceRef = useRef<HTMLSelectElement>(null);
  const [resolutionNotice, setResolutionNotice] = useState<string>();
  useEffect(() => { if (resolutionNotice) replayChoiceRef.current?.focus(); }, [resolutionNotice]);
  ...
  useEffect(() => {
    ...
    reviewedSource.current = undefined;
    setRestoredSubmission(readPendingReplay());
  ...
  useEffect(() => {
    ...
    if (!current || current.mode !== "execute") return;
    const evidence = Object.fromEntries(Object.entries(current).filter(([key]) => key !== "presentation"));
    const previous = reviewedSource.current;
    if (previous && sameJson(Object.fromEntries(Object.entries(previous).filter(([key]) => key !== "presentation")), evidence)) return;
    reviewedSource.current = current;
    setTargetScope(executionScope(current).join(", ")); resetReview();
  }, [detailQuery.data]);
  ...
  const requestPreflight = () => {
    ...
    try {
      setResolutionNotice(undefined); resetReview(); const generation = preflightGeneration.current;
  ...
  const resolutionMutation = useMutation({
    mutationFn: async (attempt: ReplayAttempt) => {
      if (!attempt.preparation) throw new Error("The saved replay preparation is unavailable. This submission remains unresolved.");
      const receipt = { sourceId: attempt.sourceId, payload: attempt.payload, preparation: attempt.preparation, submissionId: attempt.submissionId };
      const result = await api.resolveReplaySubmission(attempt.sourceId, attempt.preparation, attempt.submissionId);
      if (!settleReplayResolution(receipt, result)) throw new Error("The service response or saved receipt did not match this submission. Keep this request and check its saved status before trying again.");
      return result;
    },
    onSuccess: (result, attempt) => {
      void client.invalidateQueries({ queryKey: ["active-jobs"] });
      client.setQueryData(["job", result.job.job_id], result.job);
      setRestoredSubmission(readPendingReplay());
      if (attempt.navigationGeneration !== navigationGeneration.current) return;
      submissionRef.current = undefined; replayMutation.reset(); resetReview();
      if (result.outcome === "existing") {
        navigate(`/runs?job=${encodeURIComponent(result.job.job_id)}`);
      } else {
        setResolutionNotice("This submission is closed. Review your settings and prepare a new replay when you are ready.");
      }
    },
    onError: (error, attempt) => {
      if (attempt.navigationGeneration === navigationGeneration.current) setLocalError(error instanceof Error ? error.message : "Submission closure could not be confirmed. The saved request is retained.");
    },
  });
  const requestReplay = () => {
    try {
      setResolutionNotice(undefined);
  ...
  return <div className="page compare-page"><PageHeader title="Compare runs" actions={<><Link className="button button-ghost button-medium" to="/compare?receiver=1">Test a lab receiver control</Link><Button variant="secondary" onClick={() => runsQuery.refetch()}><RotateCcw/>Refresh history</Button></>} />
    ...
    <div className="compare-top-grid"><Panel className="history-panel"><PanelHeader title="Choose runs to compare" actions={<Badge>{selected.length} selected</Badge>} /><div className="history-search"><Search/><input aria-label="Search run history" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search name or run ID"/></div><div className="run-select-list">{runs.map((run) => <label key={run.run_id}><input type="checkbox" checked={selected.includes(run.run_id)} onChange={(event) => changeSelection(run.run_id, event.target.checked)}/><span><strong>{runLabel(source?.run_id === run.run_id ? source : run)}</strong><small>{formatDate(run.created_at)} · {run.steps.length} steps</small></span><span><Badge tone={run.mode === "execute" ? "warning" : "info"}>{sentence(run.mode)}</Badge><Badge tone={run.status === "failed" ? "danger" : "neutral"}>{sentence(run.status)}</Badge></span></label>)}</div>{!runs.length ? <EmptyState title="No run records" description="Run an experiment to create your first result. Simulate is available without a lab." /> : null}<footer>{selected.length < 2 ? <p className="workspace-note">Select at least two runs. The first selected run is the comparison baseline.</p> : null}<Button variant="primary" onClick={() => compareMutation.mutate({ runIds: [...selected], generation: comparisonGeneration.current })} disabled={selected.length < 2 || compareMutation.isPending}><GitCompareArrows/>{compareMutation.isPending ? "Comparing" : "Compare selected"}</Button></footer></Panel>
      <Panel className="replay-panel"><PanelHeader title="Prepare another run" detail="Keep the original result and test the same experiment with a deliberate change." /><div className="replay-body">{resolutionNotice ? <Callout title="Submission closed">{resolutionNotice}</Callout> : null}<fieldset className="replay-form" disabled={Boolean(uncertainSubmission) || replayMutation.isPending}><Field label="Source run"><select value={sourceId} onChange={(event) => { const selectedSource = event.target.value; setSearchParams((current) => { const next = new URLSearchParams(current); if (selectedSource) next.set("source", selectedSource); else next.delete("source"); next.delete("replay"); return next; }); }} disabled={replayMutation.isPending}><option value="">Choose a completed run</option>{sourceId && !runsQuery.data.runs.some((run) => run.run_id === sourceId) ? <option value={sourceId}>{sourceId}</option> : null}{runsQuery.data.runs.map((run) => <option value={run.run_id} key={run.run_id}>{runLabel(source?.run_id === run.run_id ? source : run)} · {sentence(run.mode)} · {formatDate(run.created_at)}</option>)}</select></Field>{sourceId && detailQuery.isError ? <ErrorState title="Source run unavailable" error={detailQuery.error} retry={() => detailQuery.refetch()} /> : null}{sourceId && detailQuery.isPending ? <LoadingState label="Loading the original experiment"/> : null}<Field label="What will change?"><select ref={replayChoiceRef} value={strategy} onChange={(event) => { setStrategy(event.target.value as ReplayStrategy); parameterDraftValidRef.current = true; setParameterDraftValid(true); resetReview(); }} disabled={!sourceId || replayMutation.isPending}>
        ...
        {strategy === "swap" ? <div className="config-grid"><Field label="Substitute node"><select value={swapStep} onChange={(event) => { setSwapStep(event.target.value); setSwapBehavior(""); resetReview(); }}><option value="">Choose a step</option>{sourceSteps.map((step) => <option key={step.step_id} value={step.step_id}>{catalogQuery.data.behaviors.find((item) => item.id === step.behavior_id)?.title ?? step.step_id}</option>)}</select></Field><Field label="Compatible behavior"><select value={swapBehavior} onChange={(event) => { setSwapBehavior(event.target.value); resetReview(); }} disabled={!swapStep}><option value="">Choose a compatible method</option>{compatible.map((item) => <option value={item.id} key={item.id}>{item.title}{platformRestriction(item.platforms)}</option>)}</select></Field></div> : null}
...
function ReplayPreflight({ report, preparation }: { report: PreflightReport; preparation?: ReplayPreparation }) {
  if (preparation) return <section aria-label="Prepared Execute replay">
    ...
    {report.plan ? <CanonicalPlanReview plan={report.plan} cleanup={report.cleanup} scope={report.scope} binding={report.approval_binding} envelope={report.approval_envelope} adaptiveAuthorization={report.adaptive_authorization}/> : null}
...
export function ComparisonResult({ comparison, names = {}, createdAt = {} }: { comparison: ComparisonResponse; names?: Record<string, string>; createdAt?: Record<string, string | undefined> }) {
  ...
  const reportContent = useMemo(() => comparisonReport(comparison, names, createdAt), [comparison, names, createdAt]);
  useEffect(() => {
      ...
      <div className="table-scroll" role="region" aria-label="Compared run outcomes" tabIndex={0}>
        <table><thead><tr><th scope="col">Run</th><th scope="col">Mode</th><th scope="col">Objective</th><th scope="col">Independent evidence</th><th scope="col">First stopped step</th><th scope="col">Cleanup</th></tr></thead>
          <tbody>{comparison.summaries.map((summary, index) => <tr key={summary.run_id}>
            <th scope="row"><Link to={runReviewPath(summary.run_id)} aria-label={summary.run_id === comparison.baseline_run_id ? "Review baseline run summary" : `Review variant ${index} run summary`}>{names[summary.run_id] ?? "Run"}</Link><small>{summary.run_id === comparison.baseline_run_id ? "Comparison baseline" : `Comparison variant ${index}`}</small>{createdAt[summary.run_id] ? <small><time dateTime={createdAt[summary.run_id]}>{formatDate(createdAt[summary.run_id]!)}</time></small> : null}</th>
