...
export function ComparePage() {
  ...
  const replayMutation = useMutation({ mutationFn: async (requestedSourceId: string) => { if (!source || source.run_id !== requestedSourceId) throw new Error("Select a source run."); assertReplayVariant(strategy, fromStep, swapStep, swapBehavior); const overrides = strategy === "parameters" ? parseParameterOverrides(parameterJson, source.scenario) : undefined; const execute = source.mode === "execute"; if (execute && (!replayConfirmed || !approvedBy.trim() || !scopeRefs.length || !executeReviewReady(replayPreflight))) throw new Error("Execute replay requires the displayed prospective review and a fresh explicit approval."); return api.replay(source.run_id, buildReplayPayload({ strategy, fromStep, swapStep, swapBehavior, profile, autonomy, provider, defenseChange, parameterOverrides: overrides, targetScope: execute ? scopeRefs : undefined, approval: execute ? { confirmed: true, approved_by: approvedBy.trim() } : undefined, actionImplementations: execute && strategy !== "exact" ? actionImplementations : undefined })); }, onSuccess: (run, requestedSourceId) => { client.invalidateQueries({ queryKey: ["runs"] }); client.setQueryData(["run", run.run_id], run); setSourceId(requestedSourceId); setSelected([requestedSourceId, run.run_id]); setComparison(undefined); setSearchParams({ source: requestedSourceId, replay: run.run_id }); setLocalError(undefined); setReplayPreflight(undefined); setReplayConfirmed(false); setApprovedBy(""); return run; }, onError: (error) => setLocalError(error instanceof Error ? error.message : "Replay was refused."), onSettled: () => { setReplayConfirmed(false); setApprovedBy(""); } });
