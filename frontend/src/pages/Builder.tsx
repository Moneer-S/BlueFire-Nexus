...
import "./Builder.css";
import { branchLabels, graphView, initialGraphLayout, inputLabel, inputTypeLabel } from "../lib/graph-view";
...
function BehaviorNode({ data, selected }: NodeProps<BehaviorFlowNode>) {
  ...
  return <article className={`flow-node tier-${behavior?.safety_tier ?? "safe"} ${selected ? "selected" : ""} ${data.invalid ? "invalid" : ""}`}>
    ...
    <strong>{behavior?.title ?? "Unavailable step"}</strong><p className="node-context">{behavior?.purpose}</p>
...
function GraphWorkspace({ behaviors, actions }: { behaviors: Behavior[]; actions: ActionDefinition[] }) {
  ...
  const displayNodes = useMemo(() => nodes.map((node) => ({ ...node, hidden: !visibleGraph.visible.has(node.id) })), [nodes, visibleGraph]);
  const displayEdges = useMemo(() => edges.map((edge) => ({ ...edge, hidden: !visibleGraph.visible.has(edge.source) || !visibleGraph.visible.has(edge.target) || (edge.data?.kind === "artifact" && !showInputs) || (edge.data?.kind === "route" && edge.data.outcome !== "success" && !allBranches && !expandedBranches.has(edge.source)), label: edge.data?.kind === "artifact" || edge.source === selectedId || edge.data?.outcome !== "success" ? edge.label : undefined })), [allBranches, edges, expandedBranches, selectedId, showInputs, visibleGraph]);
  ...
  return <div className={`page builder-page workbench-builder ${focusMode ? "builder-focus" : ""} ${showInputs ? "show-inputs" : ""}`} onKeyDownCapture={keyboard}>
