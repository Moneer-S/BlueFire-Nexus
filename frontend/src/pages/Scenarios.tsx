...
export function ScenariosPage() {
  ...
  const searchHistories = histories.filter((_query, index) => searchingHistoryIds.includes(historyIds[index]!));
  const searchPending = Boolean(search.trim()) && ([query, versionsQuery, ...searchHistories].some(resource => resource.isPending || resource.isFetching));
  const historySearchErrors = searchHistories.filter(resource => resource.isError);
  const searchIncomplete = Boolean(search.trim()) && (searchPending || historySearchErrors.length > 0 || query.isError || versionsQuery.isError);
  ...
  const apply = (replacement: Replacement) => {
    ...
    setNotice(`${replacement.action === "Open" ? "Opened" : replacement.action === "Import" ? "Imported" : replacement.action === "Duplicate" ? "Duplicated" : "Created"} ${displayTitle(replacement.document.title)}${replacement.clean ? "." : " as a local draft."}`);
  ...
  const renderRecord = (record: typeof records[number]) => {
    ...
    return <article className="experiment-row" key={key} ref={key === selectedKey ? selectedRef : undefined} tabIndex={-1} aria-label={`${displayTitle(item.title)} - ${label}`} aria-current={key === selectedKey ? "true" : undefined}>
        <div className="experiment-summary"><h2><button type="button" onClick={() => choose(record)}>{displayTitle(item.title)}</button></h2>{item.purpose ? <p>{item.purpose}</p> : <p>Add the first step to begin.</p>}<div className="experiment-meta"><span>{label === "Packaged" ? "Lab example" : label}{working ? " · Current working copy" : ""}</span><span>{item.steps.length} {item.steps.length === 1 ? "Step" : "Steps"} · {item.edges.length} routes</span></div>{key === selectedKey && !matches(record) ? <p>Selected experiment · outside this search</p> : null}
  ...
  return <div className="page experiments-page">
    ...
    {searchPending ? <p role="status">Searching experiments and saved version history. Results shown so far may be incomplete.</p> : null}
    {historySearchErrors.length ? <div role="alert" className="experiments-unavailable"><div><strong>Saved version search incomplete</strong><p>Some saved history could not be loaded. Matching older names or purposes may be missing; your working draft is preserved.</p></div><Button variant="secondary" onClick={() => { for (const resource of historySearchErrors) void resource.refetch(); }}>Retry saved history search</Button></div> : null}
    ...
    <section className="experiments-list" aria-label="Experiments">
      {groups.map((group) => {
        ...
        return <section className="experiment-identity" aria-label={`Versions of ${displayTitle((latest ?? current[0] ?? examples[0] ?? group.records[0])!.document.title)}`} key={group.id}>
      ...
      {!visible.length && !searchIncomplete ? <p className="experiments-empty">No experiments match this search. <button type="button" onClick={() => { const next = new URLSearchParams(params); next.delete("q"); setParams(next, { replace: true }); }}>Clear search</button></p> : null}
    </section>
