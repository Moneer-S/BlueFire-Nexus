...
function HistoricalRunReview({ run, catalog }: { run: RunRecord; catalog: CatalogResponse }) {
  return <div className="page runs-page">
    <PageHeader title={runLabel(run)} description={`${sentence(run.mode)} · ${sentence(run.status)} · ${formatDate(run.finalized_at ?? run.created_at)}`} actions={<><RunNameControl key={run.run_id} run={run}/><Link to="/runs"><Button variant="secondary">Back to run workspace</Button></Link><Link to={detectionLink(run.run_id)}><Button variant="secondary">Open Detection Lab</Button></Link><Link to={comparisonLink(run.run_id)}><Button variant={typeof run.replay?.source_run_id === "string" ? "secondary" : "primary"}>Replay & compare</Button></Link>{typeof run.replay?.source_run_id === "string" ? <Link className="button button-primary button-medium" to={comparisonLink(run.replay.source_run_id, run.run_id)}>Compare with original run</Link> : null}</>} />
