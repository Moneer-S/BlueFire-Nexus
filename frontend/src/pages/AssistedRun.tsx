...
export function AssistedRunReview({ jobId }: { jobId: string }) {
  ...
  return <div className="page runs-page">
    ...
    {envelope?.run_job ? <Link className="button button-primary button-medium" to={`/runs?job=${encodeURIComponent(envelope.run_job.job_id)}`}>{envelope.run_job.state === "awaiting_approval" ? "Review Execute approval" : "Open run progress"}<ArrowRight /></Link> : null}
    {envelope?.inspection ? <section aria-label="Evidence inspection"><h2>{envelope.inspection.status === "insufficient" ? "Not enough evidence" : "What the evidence shows"}</h2><p>{envelope.inspection.summary}</p><DataList items={[{ label: "Independent observations", value: envelope.inspection.observed_records }, { label: "Total records", value: envelope.inspection.total_records }]} /><ul>{envelope.inspection.findings.map((finding, index) => <li key={index}>{finding.claim}<details><summary>Evidence references</summary>{finding.evidence_refs.map((reference) => <code key={reference}>{reference}</code>)}</details></li>)}</ul>{envelope.inspection.limitations.map((limitation, index) => <p key={index}>{limitation}</p>)}<Link to={`/runs/${encodeURIComponent(envelope.inspection.run_id)}`}>Review run evidence and export<ArrowRight /></Link></section> : null}
