...
export function RunnerProfilesPage() {
  ...
  return <div className="page"><PageHeader eyebrow="Authorization envelopes" title="Runner profiles" description="Profiles bind environment, scope, capabilities, safety tiers, approvals, budgets, and cleanup before a task can reach a runner." actions={<Dialog.Root open={creating} onOpenChange={setCreating}><Dialog.Trigger asChild><Button variant="primary"><Plus/>New profile</Button></Dialog.Trigger><Dialog.Portal><Dialog.Overlay className="dialog-overlay"/><Dialog.Content className="dialog-content"><Dialog.Title>Draft runner profile</Dialog.Title><Dialog.Description>Start from an existing configuration and review its methods, access and limits. Saving does not activate it or authorize a run.</Dialog.Description><ProfileForm profiles={query.data.runner_profiles} actions={query.data.actions} onCreate={async (profile) => { await saveMutation.mutateAsync(profile); setCreating(false); }} saving={saveMutation.isPending}/></Dialog.Content></Dialog.Portal></Dialog.Root>} />
    ...
    <div className="profile-grid">{profiles.map(({ profile, resource }) => { const chmodEligible = Boolean(resource) && resource?.status !== "active" && profile.mode === "execute" && profile.platforms.length === 1 && profile.platforms[0] === "linux" && profile.enabled_actions.includes("sandbox.permission.chmod.v1"); const chmodActive = Boolean(resource) && resource?.status === "active" && profile.mode === "execute" && profile.platforms.length === 1 && profile.platforms[0] === "linux" && profile.enabled_actions.includes("sandbox.permission.chmod.v1"); return <Panel className="profile-card" aria-label={`${profileLabel(profile.id)} (${profile.id})`} key={profile.id}><header><span className={`registry-icon ${profile.mode === "execute" ? "tier-controlled" : "tier-safe"}`}>{profile.mode === "execute" ? <ShieldCheck/> : <Archive/>}</span><div><h2>{profileLabel(profile.id)}</h2><p>{sentence(profile.environment_type)}</p></div><div className="row-badges"><Badge tone={profile.mode === "execute" ? "warning" : "info"}>{profile.mode}</Badge><Badge tone={resource?.status === "active" ? "success" : resource?.status === "inactive" ? "neutral" : "warning"}>{resource ? sentence(resource.status) : "Configuration baseline"}</Badge></div></header><DataList items={[{ label: "Profile ID", value: <code>{profile.id}</code> }, { label: "Scope", value: profile.scope.join(", ") }, { label: "Platforms", value: profile.platforms.join(", ") }, { label: "Safety tiers", value: profile.safety_tiers.join(", ") }, { label: "Approval", value: profile.approval_required ? "Required" : "Not required" }, { label: "Cleanup", value: sentence(profile.cleanup_policy) }, ...(profile.platforms.length === 1 && profile.platforms[0] === "linux" && profile.enabled_actions.includes("sandbox.permission.chmod.v1") ? [{ label: "GNU chmod binding", value: profile.native_tool_installations?.some((item) => item.adapter_id === "sandbox.permission.chmod.v1") ? "Saved · inspect again before Execute" : "Not configured" }] : [])]} /><div className="budget-strip"><span><small>Seconds</small><strong>{profile.budgets.max_seconds ?? "Not reported"}</strong></span><span><small>Steps</small><strong>{profile.budgets.max_steps ?? "Not reported"}</strong></span><span><small>Bytes</small><strong>{profile.budgets.max_bytes ? `${Math.round(profile.budgets.max_bytes / 1_048_576)} MiB` : "Not reported"}</strong></span></div>{chmodActive ? <p className="field-note">Deactivate this profile before changing its GNU chmod binding.</p> : null}<footer><Badge tone="neutral">{profile.capabilities.length} capabilities</Badge><Badge tone="neutral">{profile.enabled_actions.length} actions</Badge>{resource?.status !== "active" ? <ProfileEditDialog profile={profile} profiles={query.data.runner_profiles} actions={query.data.actions} onSave={async (next) => { const { secrets, ...safe } = next; void secrets; await api.saveResource("runner-profiles", next.id, safe as unknown as Record<string, unknown>, "draft"); await queryClient.invalidateQueries({ queryKey: ["resources", "runner-profiles"] }); await queryClient.invalidateQueries({ queryKey: ["catalog"] }); setNotice(`${next.id} saved as a draft. It remains inactive until explicitly activated.`); }} /> : null}{chmodEligible ? <NativeToolSetupDialog profile={profile} triggerLabel={`Set up GNU chmod for ${profileLabel(profile.id)} (${profile.id})`} onSave={async (next) => { const { secrets, ...safe } = next; void secrets; await api.saveResource("runner-profiles", next.id, safe as unknown as Record<string, unknown>, "draft"); await queryClient.invalidateQueries({ queryKey: ["resources", "runner-profiles"] }); await queryClient.invalidateQueries({ queryKey: ["catalog"] }); await queryClient.invalidateQueries({ queryKey: ["runner-probe", next.id] }); setNotice(`${next.id} saved as a draft with a verified GNU chmod binding.`); }} /> : null}{resource ? <Button size="small" variant={resource.status === "active" ? "ghost" : "secondary"} aria-label={`${resource.status === "active" ? "Deactivate" : "Validate & activate"} ${profileLabel(profile.id)} (${profile.id})`} disabled={lifecycleMutation.isPending} onClick={() => lifecycleMutation.mutate({ id: profile.id, action: resource.status === "active" ? "deactivate" : "activate" })}><Power/>{resource.status === "active" ? "Deactivate" : "Validate & activate"}</Button> : null}</footer></Panel>; })}</div>
  </div>;
}
...
export function ProfileForm({ profiles, actions, initialProfile, onCreate, saving }: { profiles: RunnerProfile[]; actions: import("../types").ActionDefinition[]; initialProfile?: RunnerProfile; onCreate: (profile: RunnerProfile) => Promise<unknown>; saving: boolean }) {
  ...
  return <form className="dialog-form" onSubmit={async (event) => {
    ...
    <Field label="Profile ID" hint={initialProfile ? "Existing profile identity is retained while you configure its methods." : undefined}><input value={id} onChange={(event) => setId(event.target.value)} pattern="[a-z](?:[a-z0-9._]|-)+" readOnly={Boolean(initialProfile)} required/></Field>
    {draft && template ? <>
