...
export function RunnerProfilesPage() {
  ...
  return <div className="page"><PageHeader eyebrow="Authorization envelopes" title="Runner profiles" description="Profiles bind environment, scope, capabilities, safety tiers, approvals, budgets, and cleanup before a task can reach a runner." actions={<Dialog.Root open={creating} onOpenChange={setCreating}><Dialog.Trigger asChild><Button variant="primary"><Plus/>New profile</Button></Dialog.Trigger><Dialog.Portal><Dialog.Overlay className="dialog-overlay"/><Dialog.Content className="dialog-content"><Dialog.Title>Draft runner profile</Dialog.Title><Dialog.Description>Start from an existing configuration and review its methods, access and limits. Saving does not activate it or authorize a run.</Dialog.Description><ProfileForm profiles={query.data.runner_profiles} actions={query.data.actions} onCreate={async (profile) => { await saveMutation.mutateAsync(profile); setCreating(false); }} saving={saveMutation.isPending}/></Dialog.Content></Dialog.Portal></Dialog.Root>} />
    ...
    <div className="profile-grid">{profiles.map(({ profile, resource }) => { const nativeTools = profile.platforms.length === 1 && profile.platforms[0] === "linux" ? NATIVE_TOOL_ACTIONS.filter(actionId => profile.enabled_actions.includes(actionId) && !profile.blocked_actions.includes(actionId)) : []; const toolSetupEligible = Boolean(resource) && resource?.status !== "active" && profile.mode === "execute"; const toolSetupActive = Boolean(resource) && resource?.status === "active" && profile.mode === "execute"; return <Panel className="profile-card" aria-label={`${profileLabel(profile.id)} (${profile.id})`} key={profile.id}><header><span className={`registry-icon ${profile.mode === "execute" ? "tier-controlled" : "tier-safe"}`}>{profile.mode === "execute" ? <ShieldCheck/> : <Archive/>}</span><div><h2>{profileLabel(profile.id)}</h2><p>{sentence(profile.environment_type)}</p></div><div className="row-badges"><Badge tone={profile.mode === "execute" ? "warning" : "info"}>{profile.mode}</Badge><Badge tone={resource?.status === "active" ? "success" : resource?.status === "inactive" ? "neutral" : "warning"}>{resource ? sentence(resource.status) : "Configuration baseline"}</Badge></div></header><DataList items={[{ label: "Profile ID", value: <code>{profile.id}</code> }, { label: "Scope", value: profile.scope.join(", ") }, { label: "Platforms", value: profile.platforms.join(", ") }, { label: "Safety tiers", value: profile.safety_tiers.join(", ") }, { label: "Approval", value: profile.approval_required ? "Required" : "Not required" }, { label: "Cleanup", value: sentence(profile.cleanup_policy) }, ...nativeTools.map(actionId => ({ label: `${NATIVE_TOOL_SETUP[actionId].name} binding`, value: profile.native_tool_installations?.some((item) => item.adapter_id === actionId) ? "Saved · inspect again before Execute" : "Not configured" }))]} /><div className="budget-strip"><span><small>Seconds</small><strong>{profile.budgets.max_seconds ?? "Not reported"}</strong></span><span><small>Steps</small><strong>{profile.budgets.max_steps ?? "Not reported"}</strong></span><span><small>Bytes</small><strong>{profile.budgets.max_bytes ? `${Math.round(profile.budgets.max_bytes / 1_048_576)} MiB` : "Not reported"}</strong></span></div>{toolSetupActive ? nativeTools.map(actionId => <p className="field-note" key={actionId}>Deactivate this profile before changing its {NATIVE_TOOL_SETUP[actionId].name} binding.</p>) : null}<footer><Badge tone="neutral">{profile.capabilities.length} capabilities</Badge><Badge tone="neutral">{profile.enabled_actions.length} actions</Badge>{resource?.status !== "active" ? <ProfileEditDialog profile={profile} profiles={query.data.runner_profiles} actions={query.data.actions} onSave={async (next) => { const { secrets, ...safe } = next; void secrets; await api.saveResource("runner-profiles", next.id, safe as unknown as Record<string, unknown>, "draft"); await queryClient.invalidateQueries({ queryKey: ["resources", "runner-profiles"] }); await queryClient.invalidateQueries({ queryKey: ["catalog"] }); setNotice(`${next.id} saved as a draft. It remains inactive until explicitly activated.`); }} /> : null}{toolSetupEligible ? nativeTools.map(actionId => <NativeToolSetupDialog key={actionId} actionId={actionId} profile={profile} triggerLabel={`Set up ${NATIVE_TOOL_SETUP[actionId].name} for ${profileLabel(profile.id)} (${profile.id})`} onSave={async (next) => { const { secrets, ...safe } = next; void secrets; await api.saveResource("runner-profiles", next.id, safe as unknown as Record<string, unknown>, "draft"); await queryClient.invalidateQueries({ queryKey: ["resources", "runner-profiles"] }); await queryClient.invalidateQueries({ queryKey: ["catalog"] }); await queryClient.invalidateQueries({ queryKey: ["runner-probe", next.id] }); setNotice(`${next.id} saved as a draft with a verified ${NATIVE_TOOL_SETUP[actionId].name} binding.`); }} />) : null}{resource ? <Button size="small" variant={resource.status === "active" ? "ghost" : "secondary"} aria-label={`${resource.status === "active" ? "Deactivate" : "Validate & activate"} ${profileLabel(profile.id)} (${profile.id})`} disabled={lifecycleMutation.isPending} onClick={() => lifecycleMutation.mutate({ id: profile.id, action: resource.status === "active" ? "deactivate" : "activate" })}><Power/>{resource.status === "active" ? "Deactivate" : "Validate & activate"}</Button> : null}</footer></Panel>; })}</div>
  </div>;
}

function ProfileEditDialog({ profile, profiles, actions, onSave }: { profile: RunnerProfile; profiles: RunnerProfile[]; actions: import("../types").ActionDefinition[]; onSave: (profile: RunnerProfile) => Promise<unknown> }) {
  const [open, setOpen] = useState(false);
  const [saving, setSaving] = useState(false);
  return <Dialog.Root open={open} onOpenChange={(next) => { if (!saving) setOpen(next); }}><Dialog.Trigger asChild><Button size="small" variant="secondary" aria-label={`Configure methods for ${profileLabel(profile.id)} (${profile.id})`}>Configure methods</Button></Dialog.Trigger><Dialog.Portal><Dialog.Overlay className="dialog-overlay"/><Dialog.Content className="dialog-content"><Dialog.Title>Configure {profileLabel(profile.id)}</Dialog.Title><Dialog.Description>Review profile <code>{profile.id}</code> and save a draft with the same identity. Saving does not activate it or authorize a run.</Dialog.Description><ProfileForm profiles={profiles} actions={actions} initialProfile={profile} onCreate={async (next) => { setSaving(true); try { await onSave(next); setOpen(false); } finally { setSaving(false); } }} saving={saving}/></Dialog.Content></Dialog.Portal></Dialog.Root>;
}

export function ProfileForm({ profiles, actions, initialProfile, onCreate, saving }: { profiles: RunnerProfile[]; actions: import("../types").ActionDefinition[]; initialProfile?: RunnerProfile; onCreate: (profile: RunnerProfile) => Promise<unknown>; saving: boolean }) {
  ...
  useEffect(() => {
    ...
    setId(initialProfile.id);
    setTemplateId(initialProfile.id);
    setDraft(structuredClone(initialProfile));
  }, [initialProfile]);
  const templateProfiles = initialProfile && !profiles.some((profile) => profile.id === initialProfile.id) ? [initialProfile, ...profiles] : profiles;
  const templates = templateProfiles.filter((profile) => profile.platforms.length && (profile.mode === "simulate" || profile.enabled_actions.some((id) => actions.some((action) => action.id === id))));
  const template = templates.find((profile) => profile.id === templateId);
  const compatible = actions.filter((action) => template && !template.blocked_actions.includes(action.id) && draft?.platforms.every((platform) => action.platforms.includes(platform)) && action.capabilities.every((capability) => draft?.capabilities.includes(capability)) && draft?.safety_tiers.includes(action.safety_tier));
  ...
  const retainedBindings = (draft?.native_tool_installations ?? []).filter((item) => selected.includes(item.adapter_id) && draft?.platforms.includes(item.platform));
  const removedBindingCount = (draft?.native_tool_installations ?? []).length - retainedBindings.length;
  ...
  return <form className="dialog-form" onSubmit={async (event) => {
    ...
    setError(undefined);
    try { await onCreate({ ...draft, id, enabled_actions: selected, native_tool_installations: retainedBindings.length ? retainedBindings : undefined, secrets: {} }); }
    ...
    <Field label="Configuration template" hint={initialProfile ? "Existing profile configuration is being edited; its identity and enrolled runner references are retained." : "Copies configuration only. Activation and run approval remain separate."}><select required value={templateId} disabled={Boolean(initialProfile)} onChange={(event) => {
    ...
    <Field label="Profile ID" hint={initialProfile ? "Existing profile identity is retained while you configure its methods." : "Use a lowercase, versioned ID, for example local-experiment.v1."}><input aria-label="Profile ID" value={id} onChange={(event) => setId(event.target.value)} pattern={"[a-z][a-z0-9]*(?:[._\\-][a-z0-9]+)*\\.v[1-9][0-9]*"} readOnly={Boolean(initialProfile)} required/></Field>
    {draft && template ? <>
      ...
      <fieldset><legend>Methods permitted by this configuration</legend>{compatible.map((action) => <label className="check-row" key={action.id}><input type="checkbox" checked={selected.includes(action.id)} onChange={(event) => update({ enabled_actions: event.target.checked ? [...draft.enabled_actions, action.id] : draft.enabled_actions.filter((id) => id !== action.id) })}/><span>{displayTitle(action.title)}</span></label>)}{!compatible.length ? <p>No executable methods match this platform and template.</p> : null}</fieldset>
      <fieldset><legend>Access from template</legend>{template.scope.map((scope) => <label className="check-row" key={scope}><input type="checkbox" checked={draft.scope.includes(scope)} onChange={(event) => update({ scope: event.target.checked ? [...draft.scope, scope] : draft.scope.filter((item) => item !== scope) })}/><span>{scope === "sandbox.workspace" ? "Experiment workspace" : scope === "network.loopback" ? "Local loopback network" : scope}</span></label>)}</fieldset>
    ...
    {removedBindingCount ? <p role="alert">Saving will remove {removedBindingCount} native tool binding{removedBindingCount === 1 ? "" : "s"} because its method or platform is no longer selected.</p> : null}
