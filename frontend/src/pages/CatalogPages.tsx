...
export function RunnersPage() {
  ...
  return <div className="page"><PageHeader eyebrow="Execution boundary" title="Runners" description="Enroll, start, inspect, stop, revoke, and remove the authenticated local runner before Execute." actions={<Dialog.Root open={creating} onOpenChange={(open) => { if (!savePending.current) setCreating(open); }}><Dialog.Trigger asChild><Button variant="primary"><Plus/>Register record</Button></Dialog.Trigger><Dialog.Portal><Dialog.Overlay className="dialog-overlay"/><Dialog.Content className="dialog-content"><Dialog.Title>Register runner record</Dialog.Title><Dialog.Description>Persist a bounded runner reference. This does not launch, enroll, authenticate, or attest a runner.</Dialog.Description><Dialog.Close asChild><button className="dialog-close" aria-label="Close dialog" disabled={saveMutation.isPending}><X/></button></Dialog.Close><RunnerRecordForm onCreate={async (runner) => { savePending.current = true; try { await saveMutation.mutateAsync(runner); setCreating(false); } finally { savePending.current = false; } }} saving={saveMutation.isPending}/></Dialog.Content></Dialog.Portal></Dialog.Root>} />
    ...
    <Field label="Experiment runner profile" hint="Select the same profile as your experiment. This selects diagnostics only; it does not change profile permissions."><select aria-label="Experiment runner profile" value={lifecycleProfileId ?? ""} disabled={lifecycleMutation.isPending} onChange={(event) => { setSearchParams(event.target.value ? { profile: event.target.value } : {}); lifecycleMutation.reset(); setNotice(undefined); }}><option value="">Choose an Execute profile</option>{executeProfiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.id} · {profile.platforms.join(", ")}</option>)}</select></Field>
    {!lifecycleProfileId ? <Callout title={requestedProfile ? "Selected profile unavailable" : "Choose an experiment profile"}>{requestedProfile ? "The linked profile is not an active Execute profile. Choose an available profile explicitly." : "Choose the profile used by your experiment to inspect its runner. No default profile was selected."}</Callout> : <>
      ...
      <Callout title="Missing a method after an application update?">The enrolled runner can still use its earlier verified binary. Stop it safely, then review Upgrade managed runner. Upgrade can be refused when prior recovery history, receipts, or other ownership checks require preservation. Keep the refusal details; do not remove trust or delete history to force it through. After an accepted upgrade, start the runner and run a fresh experiment preflight.</Callout>
    </>}
...
function ManagedRunnerPanel({ status, loading, failed, profileId, busy, onRefresh, onAction }: { status?: RunnerLifecycleStatus; loading: boolean; failed: boolean; profileId?: string; busy: boolean; onRefresh: () => void; onAction: (action: ManagedRunnerAction, confirmation?: string) => void }) {
  ...
  return <Panel>
    <PanelHeader eyebrow="Authenticated local host" title="Managed runner lifecycle" detail="Lifecycle actions are explicit. Status and probes never install or start the runner." actions={status ? <Badge tone={tone} dot>{status.state === "ready" ? "Authenticated" : sentence(status.state)}</Badge> : null}/>
    {loading ? <LoadingState label="Checking managed runner status"/> : failed ? <Callout tone="warning" title="Managed status unavailable">No lifecycle action was attempted. Refresh status or inspect the local service.</Callout> : status ? <>
      ...
      <footer className="dialog-actions">
        ...
        {canUpgrade ? <Modal title="Upgrade managed runner" description="Verify the packaged runner and update its managed bootstrap while preserving the existing enrollment and sandbox." trigger={<Button size="small" variant="secondary" disabled={busy || !profileId}><Box/>Upgrade managed runner</Button>}><div className="dialog-form"><Callout tone="warning" title="Explicit verified upgrade">BlueFire will refuse an unverified package, running host, active task, unresolved receipt, or unsafe replacement. The existing enrollment and allowed profile identities must match. Prior execution recovery history can also block an upgrade; no history is removed by this operation.</Callout><div className="dialog-actions"><Dialog.Close asChild><Button type="button" variant="ghost">Cancel</Button></Dialog.Close><Dialog.Close asChild><Button type="button" variant="secondary" disabled={busy || !profileId} onClick={() => onAction("upgrade")}><Box/>Confirm verified upgrade</Button></Dialog.Close></div></div></Modal> : null}
