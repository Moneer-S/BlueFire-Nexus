  ...
  return <div className="page">
    ...
    <div className="catalog-layout">
      <Panel className="catalog-list-panel">
        ...
        <div className="behavior-table" role="list">{behaviors.map((item) => <button role="listitem" className={selected?.id === item.id ? "selected" : ""} onClick={() => setSelectedId(item.id)} key={item.id}><span className={`registry-icon tier-${item.safety_tier}`}><FileCode2/></span><span><strong>{item.title}</strong><small>{item.platforms.join(" · ")}</small><em>{item.purpose}</em></span><span className="row-badges"><Badge tone={tierTone(item.safety_tier)}>{item.safety_tier}</Badge><Badge tone={item.execution_state === "action" ? "success" : item.execution_state === "simulation" ? "info" : "neutral"}>{item.execution_state === "action" ? "Executable method" : "Research only"}</Badge></span></button>)}</div>
...
function BehaviorDetail({ behavior, actions }: { behavior: Behavior; actions: import("../types").ActionDefinition[] }) {
  return <Panel className="detail-panel"><PanelHeader title={behavior.title} detail={behavior.purpose} actions={<Badge tone={tierTone(behavior.safety_tier)}>{behavior.safety_tier}</Badge>} />
...
export function ProfileForm({ profiles, actions, onCreate, saving }: { profiles: RunnerProfile[]; actions: import("../types").ActionDefinition[]; onCreate: (profile: RunnerProfile) => Promise<unknown>; saving: boolean }) {
  ...
  return <form className="dialog-form" onSubmit={async (event) => {
    ...
    {missingCleanup.length ? <p role="alert">Select the cleanup method required by {missingCleanup.map((action) => action.title).join(", ")} before saving.</p> : null}
...
export function ActionsPage() {
  ...
  return <div className="page"><PageHeader eyebrow="Installed capability" title="Actions & plugins" description="Reviewed, versioned action contracts are the only path from orchestration intent to runner effects." actions={<Modal title="Add plugin manifest" description="Persist a strict declarative manifest. This does not download, verify bytes, install code, or register dynamic actions." trigger={<Button variant="primary"><Plus/>Add manifest</Button>}><PluginManifestForm onCreate={(plugin) => saveMutation.mutate(plugin)} saving={saveMutation.isPending}/></Modal>} />
    ...
    <Panel><div className="catalog-filters"><label className="search-box"><Search/><span className="sr-only">Search actions</span><input aria-label="Search actions" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search actions or capabilities" /></label><Badge tone="success">{actions.length} registered</Badge></div><div className="table-scroll"><table><thead><tr><th>Action</th><th>Platforms</th><th>Safety</th><th>Effects</th><th>Cleanup</th></tr></thead><tbody>{actions.map((item) => <tr key={item.id}><td><strong>{item.title}</strong><code>{item.id}</code><small>{item.purpose}</small></td><td>{item.platforms.join(", ")}</td><td><Badge tone={tierTone(item.safety_tier)}>{item.safety_tier}</Badge></td><td><Badge tone={item.mutates ? "warning" : "success"}>{item.capabilities.includes("network.loopback") ? "Local network transfer" : item.mutates ? "Writes files" : item.capabilities.includes("process.spawn") ? "Starts a process" : item.capabilities.includes("native.execution") ? "Local computation" : "Reads data"}</Badge></td><td>{item.cleanup_action_id ? <code>{item.cleanup_action_id}</code> : item.id.includes("cleanup") ? "Self" : "Not declared"}</td></tr>)}</tbody></table></div></Panel>
