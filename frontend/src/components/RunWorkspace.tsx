...
export function RunWorkspace({ embedded }: { embedded?: { job: RunJob; releaseEnabled: boolean; controlsEnabled?: boolean; onCancelRequested?: () => void } } = {}) {
  ...
  return <div className="page runs-page">
    <PageHeader title={linkedJobId ? "Run details" : "Runs"} actions={<><Link className="button button-primary button-medium" to="/runs?prepare=1" onClick={() => setSetupExpanded(true)}>Review new run</Link>{activeRun ? <Link className="button button-secondary button-medium" to={runReviewPath(activeRun.run_id)}>Review latest result</Link> : null}</>} />
