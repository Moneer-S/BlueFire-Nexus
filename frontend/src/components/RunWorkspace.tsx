...
function LiveConsole({ run, job, events, pending, config, approvalPreflight, approvalRequest, proposalReview, approvalConfirmed, approvedBy, approvalPending, releaseEnabled, controlActionsEnabled, controlPending, mutableControlsEnabled, retryEnabled, onApprovalConfirmed, onApprovedBy, onApprove, onControl, onRetry, onProposalDecision, onProposalReviewLoaded, onReview }: LiveConsoleProps) {
  ...
  const retainedReview = (job?.request?._run_submission_preflight ?? (job?.request?.replay_preparation as { preflight?: PreflightReport } | undefined)?.preflight) as PreflightReport | undefined;
  const jobMode = run?.mode ?? (job?.request?.mode === "execute" ? "execute" : job?.request?.mode === "simulate" ? "simulate" : retainedReview?.plan?.mode === "execute" ? "execute" : retainedReview?.plan?.mode === "simulate" ? "simulate" : config.mode);
