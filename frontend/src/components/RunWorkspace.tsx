...
export function RunWorkspace({ embedded }: { embedded?: { job: RunJob; releaseEnabled: boolean; controlsEnabled?: boolean; onCancelRequested?: () => void } } = {}) {
  ...
  const runnerLifecycleQuery = useQuery({ queryKey: ["runner-lifecycle"], queryFn: () => api.runnerStatus(), enabled: !isEmbedded });
  ...
  const preflightCanAuthorize = runConfig.mode === "execute" ? hasExecutePlanReview(preflight) : preflight?.ready === true;
  ...
  return <div className="page runs-page">
    <PageHeader title={linkedJobId ? "Run details" : "Runs"} actions={<><Link className="button button-primary button-medium" to="/runs?prepare=1" onClick={() => setSetupExpanded(true)}>Review new run</Link><Link className="button button-secondary button-medium" to="/runs?saved=1">Run saved with Assistant</Link>{activeRun ? <Link className="button button-secondary button-medium" to={runReviewPath(activeRun.run_id)}>Review latest result</Link> : null}</>} />
    {!linkedJobId && !showLiveWorkspace ? <RunHistoryPanel runs={runsQuery.data?.runs} unavailable={runsQuery.data?.unavailable_run_count ?? 0} pending={runsQuery.isPending} error={runsQuery.error} retry={() => runsQuery.refetch()} /> : null}
    ...
    <>
      ...
      {runConfig.mode === "execute" && preflight && (preflight.ready || preflight.status === "approval_required") && !hasExecutePlanReview(preflight) ? <Callout tone="danger" title="Complete run review unavailable">Job creation remains disabled until the service returns the complete plan, scope binding, and approval envelope.</Callout> : null}
...
function JobApprovalGate({ job, preflight: ordinaryPreflight, approvalRequest, proposalReview, confirmed, approvedBy, pending, releaseEnabled, onConfirmed, onApprovedBy, onApprove }: { job: RunJob; preflight?: PreflightReport; approvalRequest: Record<string, unknown> | null; proposalReview?: AIProposalReview; confirmed: boolean; approvedBy: string; pending: boolean; releaseEnabled: boolean; onConfirmed: (confirmed: boolean) => void; onApprovedBy: (identity: string) => void; onApprove: () => void }) {
...
function Timeline({ run, job, events: incrementalEvents, pending }: { run: RunRecord | null; job: RunJob | null; events: RunEventPage["items"]; pending: boolean }) {
  ...
  return <ol className="timeline">{pending && !events.length ? <li><span className="timeline-dot planning"/><div><strong>Planning request submitted</strong><p>Waiting for the service to accept the request. Its saved status will appear here.</p></div><time>Now</time></li> : events.map((event: any, index) => <li key={Number(event.sequence ?? index + 1)}><span className={`timeline-dot ${normalizeStatus(String(event.type ?? event.event_type ?? event.status ?? "observed"))}`}/><div><strong>{sentence(String(event.type ?? event.event_type ?? event.status ?? "event"))}</strong><p>{event.step_id ? `${event.step_id} · ` : ""}{sentence(String(event.disposition ?? event.message ?? "Canonical record"))}</p></div><time>{event.timestamp ? formatDate(event.timestamp) : `#${event.sequence ?? index + 1}`}</time></li>)}</ol>;
}
