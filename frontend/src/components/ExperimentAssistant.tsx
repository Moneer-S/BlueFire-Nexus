...
import { isGraphRequest, isGraphSelection, isSavedGraphRequest, isSavedGraphSelection } from "../lib/assistance";
...
export function ExperimentAssistant({ providers }: { providers: NonNullable<CatalogResponse["ai"]["providers"]> }) {
  ...
  const context = useQuery({ queryKey: ["assistance-context", savedGraphSelection ? "saved_graph" : graphSelection ? "graph" : "detection", savedGraphSelection?.selected, graphSelection?.baseScenario, detectionSelection?.runId, detectionSelection?.candidateId, detectionSelection?.resourceDigest],
    queryFn: () => savedGraphSelection ? api.assistanceRunContext(savedGraphSelection.selected) : graphSelection ? api.assistanceGraphContext(graphSelection.baseScenario) : api.assistanceContext(detectionSelection!.runId, detectionSelection!.candidateId),
  ...
  const current = Boolean(selected && (isSavedGraphSelection(selected) ? savedGraphSelection && sameJson(selected, savedGraphSelection.selected) : isGraphSelection(selected) ? graphSelection && sameJson(selected.base_scenario, graphSelection.baseScenario)
  ...
  const start = () => {
    ...
    const request: AssistanceRequest = savedGraphSelection ? { ...common, selection: savedGraphSelection.selected } : graphSelection ? { ...common, selection: { kind: "graph", base_scenario: graphSelection.baseScenario } }
    ...
    <Dialog.Portal><Dialog.Overlay className="assistant-overlay" /><Dialog.Content className="experiment-assistant" aria-describedby="assistant-description">
      ...
      <div className="assistant-body">
        ...
        {receipt ? <section className="assistant-operation" aria-label="Saved assistant work">
          ...
          {turn ? <>
            ...
            {turn.results.length ? <section className="assistant-results" aria-label="Saved results"><h4>Saved results</h4>{turn.results.map((result) => {
              ...
              return <div key={`${result.step_id}:${result.kind}`}><strong>{result.kind === "run_inspected" ? "Run and evidence review" : result.kind === "graph_saved" ? "Experiment saved" : result.kind === "detection_revision" ? "Rule revision and evaluation" : "Method comparison"}</strong>{result.kind === "run_inspected" ? <p>{sentence(result.mode)} · {result.observed_records} independently observed records · {result.inspection_status === "insufficient" ? "Not enough evidence" : "Evidence reviewed"} · Cleanup: {sentence(result.cleanup_state)}{result.runtime_modified ? " · Includes reviewed runtime changes" : ""}</p> : result.kind === "graph_saved" ? <p>Version {result.version} · {result.operator_modified ? "Includes your edits · " : ""}Not run</p> : <p>{result.evaluation_ids.length} evaluation{result.evaluation_ids.length === 1 ? "" : "s"} · {result.run_ids.length} run{result.run_ids.length === 1 ? "" : "s"}</p>}{path ? <Link onClick={navigate} to={path}>Inspect saved result<ArrowRight aria-hidden="true" /></Link> : null}{result.kind === "graph_saved" ? <Link onClick={navigate} to={`/runs?graph_job=${encodeURIComponent(result.proposal_job_id)}`}>Run with Assistant<ArrowRight aria-hidden="true" /></Link> : null}</div>;
            })}</section> : null}
        ...
        {!receipt ? <section className="assistant-composer" aria-label="Assistant controls">
          ...
          {!receipt ? <form onSubmit={(event) => { event.preventDefault(); start(); }}><Field label="What would you like to do?"><textarea rows={4} maxLength={1000} value={message} onChange={(event) => updateMessage(event.target.value)} placeholder={savedGraphSelection ? "Run this saved experiment with the selected settings, then explain its outcomes, observations, and cleanup." : graphSelection ? "Build an experiment to discover the lab system, collect owned test records, and verify cleanup." : "Improve this rule from the selected run, then try another method and compare detections."} /></Field><div className="assistant-send-row">{!graphSelection && !savedGraphSelection ? <Field label="Evidence case"><select value={caseRole} onChange={(event) => setCaseRole(event.target.value as DetectionCaseRole)}><option value="attack">Attack</option><option value="benign">Benign</option><option value="replay">Replay</option><option value="heldout">Held-out</option></select></Field> : <p>{savedGraphSelection ? "Use the settings selected in Runs. Evidence review follows the saved run." : "Proposal and validation only. Saving waits for your review."}</p>}<Button type="submit" variant="primary" disabled={!ready || submit.isPending || DEMO_MODE}>Start work<ArrowRight aria-hidden="true" /></Button></div></form> : null}
        </section> : null}
