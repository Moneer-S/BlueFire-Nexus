        ...
        {receipt ? <section className="assistant-operation" aria-label="Saved assistant work">
          ...
          {turn ? <>
            ...
            {turn.results.length ? <section className="assistant-results" aria-label="Saved results"><h4>Saved results</h4>{turn.results.map((result) => {
              ...
              return <div key={`${result.step_id}:${result.kind}`}><strong>{result.kind === "run_inspected" ? "Run and evidence review" : result.kind === "graph_saved" ? "Experiment saved" : result.kind === "detection_revision" ? "Rule revision and evaluation" : "Method comparison"}</strong>{result.kind === "run_inspected" ? <p>{sentence(result.mode)} · {result.observed_records} independently observed records · {result.inspection_status === "insufficient" ? "Not enough evidence" : "Evidence reviewed"} · Cleanup: {sentence(result.cleanup_state)}</p> : result.kind === "graph_saved" ? <p>Version {result.version} · {result.operator_modified ? "Includes your edits · " : ""}Not run</p> : <p>{result.evaluation_ids.length} evaluation{result.evaluation_ids.length === 1 ? "" : "s"} · {result.run_ids.length} run{result.run_ids.length === 1 ? "" : "s"}</p>}{path ? <Link onClick={navigate} to={path}>Inspect saved result<ArrowRight aria-hidden="true" /></Link> : null}{result.kind === "graph_saved" ? <Link onClick={navigate} to={`/runs?graph_job=${encodeURIComponent(result.proposal_job_id)}`}>Run with Assistant<ArrowRight aria-hidden="true" /></Link> : null}</div>;
            })}</section> : null}
