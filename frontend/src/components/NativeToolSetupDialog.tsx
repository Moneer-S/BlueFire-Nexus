...
export function NativeToolSetupDialog({ profile, onSave }: { profile: RunnerProfile; onSave: (profile: RunnerProfile) => Promise<unknown> }) {
  ...
  return <Dialog.Root open={open} onOpenChange={(next) => { setOpen(next); if (!next) { requestRef.current += 1; setInspecting(false); setCandidate(undefined); setError(undefined); } }}>
    <Dialog.Trigger asChild><Button size="small" variant="secondary">Set up GNU chmod</Button></Dialog.Trigger>
    <Dialog.Portal><Dialog.Overlay className="dialog-overlay"/><Dialog.Content className="dialog-content">
      ...
      <Dialog.Description>Inspect one protected Linux installation and attach its verified identity to this draft profile. Inspection does not execute chmod or activate the profile.</Dialog.Description>
