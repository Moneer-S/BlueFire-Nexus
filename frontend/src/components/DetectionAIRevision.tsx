      ...
      {!jobId ? <>
        ...
        {proposalJob ? <>
          ...
          {proposal ? <>
            ...
            <p>{proposal.reason}</p>
            <div className="detection-source-comparison"><div><h5>Original · revision {resource?.document.revision ?? "at request"}</h5><pre aria-label="Original rule source">{proposal.parent.source}</pre></div><div><h5>Proposed revision</h5><pre aria-label="Proposed rule source">{proposal.source}</pre></div></div>
            <p>{proposal.evidence_refs.length} independent observations informed this proposal. <Link to={`/runs/${encodeURIComponent(proposal.source_run.run_id)}`}>Inspect source observations</Link></p>
            ...
            {!current || manualEdits ? <Callout tone="warning" title="Review context changed">{manualEdits ? "There are unsaved manual edits. Save or restore them before accepting this proposal." : "The selected rule or source run differs from this proposal. Its original binding remains unchanged."}{boundLink ? <Link to={boundLink}>Open this proposal's rule and source</Link> : null}</Callout> : null}
