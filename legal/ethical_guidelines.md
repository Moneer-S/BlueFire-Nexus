...
- Never place production credentials, customer identifiers, or proprietary endpoints in `config.yaml`, docs, tests, or sample payloads.
- Keep AI and telemetry integrations disabled by default and only enable with explicit user-provided keys in local environment variables.

## Runtime Safety Requirements

- `general.dry_run` should remain `true` unless you intentionally run in a controlled lab.
- `general.safeties.allowed_subnets` must be populated with approved private ranges before enabling network-touching operations.
- Do not enable destructive simulations unless the per-operation acknowledgement flag is set and the environment owner has approved the scope.
