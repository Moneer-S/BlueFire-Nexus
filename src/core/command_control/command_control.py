...
class CommandControl:
    ...
    def run_operation(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if handler:
            try:
                ...
                self.logger.error(
                    f"Error during C2 operation '{operation_type}': {e}", exc_info=True
                )
    ...
    def _beacon_worker(self, beacon_id: str, stop_event: threading.Event, details: Dict[str, Any]):
        ...
        if interval <= 0:
            self.logger.warning(
                f"[Beacon {beacon_id}] Invalid interval ({interval}), defaulting to 60s."
            )
            interval = 60
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        if not response.text or not response.headers.get(
                            "content-type", ""
                            ...
                            if isinstance(tasks, list) and tasks:  # Check if it's a non-empty list
                                ...
                                self.logger.info(
                                    f"[Beacon {beacon_id}] Received task from C2: {task_to_execute}"
                                )
                        ...
                        self.logger.debug(
                            f"[Beacon {beacon_id}] Response was not valid JSON, no tasks received."
                        )
                    except Exception as task_err:
                        self.logger.warning(
                            f"[Beacon {beacon_id}] Could not parse tasks from response: {task_err}"
                        )
                    # ---------------------------
                ...
                self.logger.error(
                    f"[Beacon {beacon_id}] Error in main beacon loop: {outer_e}", exc_info=True
                )
    ...
    def _handle_http_beacon(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if beacon_id in self.beacon_threads:
            ...
            return {
                "status": "skipped",
                "message": f"Beacon '{beacon_id}' already active.",
                "beacon_id": beacon_id,
            }
        ...
        thread = threading.Thread(
            target=self._beacon_worker, args=(beacon_id, stop_event, details), daemon=True
    ...
    def _handle_stop_beacon(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if stop_event:
            ...
            return {
                "status": "success",
                "message": f"Stop signal sent to beacon '{beacon_id}'.",
                "beacon_id": beacon_id,
            }
        else:
            ...
            return {
                "status": "skipped",
                "message": f"Beacon '{beacon_id}' not found or already stopped.",
                "beacon_id": beacon_id,
            }
    ...
    def _handle_not_implemented(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.warning(
            f"C2 technique '{technique_name}' is not yet implemented with realistic actions."
        )
        return {
            "status": "skipped",
    ...
    def _handle_proxy(self, data: Dict[str, Any]) -> Dict[str, Any]:
        return self._handle_not_implemented(data)
    ...
    def _execute_received_task(self, task: Dict[str, Any]):
        ...
        if not module or not operation_data:
            self.logger.error(
                f"Task {task_id} is invalid (missing module or operation_data). Skipping."
            )
            result_payload["result_data"] = {"error": "Invalid task format received."}
        elif not self.nexus:
            self.logger.error(
                f"Nexus instance not available in C2 module. Cannot execute task {task_id}."
            )
            ...
            try:
                ...
                self.logger.error(
                    f"Exception executing task {task_id} ({module}): {e}", exc_info=True
                )
...
if __name__ == "__main__":
    ...
    try:
            ...
            if request.method == "POST":
                try:
                    ...
                    if "exfil_data" in received_data:
                        ...
                        mock_c2_logger.info(
                            f"---> Received {len(exfil_chunks_received)} exfil chunks via POST."
                        )
                    ...
                    if "results" in received_data:
                        mock_c2_logger.info(
                            f"---> Received {len(received_data['results'])} task results via POST."
                        )
                ...
                for key in ["results", "exfil_data"]:
                    if key in received_data:
                        try:
                            ...
                            mock_c2_logger.info(
                                f"---> Received {len(decoded_data)} {key} items via GET param."
                            )
                        except Exception as decode_err:
                            mock_c2_logger.warning(f"Could not decode {key} from GET: {decode_err}")
        ...
        server_thread = threading.Thread(
            target=run_mock_server, daemon=True, name="MockC2ServerThread"
        )
        server_thread.start()
