...
class LateralMovement:
    ...
    def _handle_psexec(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            output += "Copyright (C) 2001-2021 Mark Russinovich\n"
        ...
        try:
            ...
            if method == "invoke_command":
                ...
                if "session_option" in data:
                    ps_cmd += " -SessionOption (New-PSSessionOption -NoMachineProfile)"
    ...
    def _handle_service_creation(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if method == "sc":
                ...
                if "start_service" in data and data["start_service"]:
                    output += "[SC] StartService RUNNING\n"
        ...
        try:
            ...
            if method == "sc":
                if action == "stop":
                    ...
                    output += "        WAIT_HINT          : 0x0\n"

            elif method == "powershell":
                if action == "query":
                    output = f"Name                : {service_name}\n"
                    output += "DisplayName         : Print Spooler\n"
                    output += "Status              : Running\n"
                    output += "DependentServices   : {Fax}\n"
                    output += "ServicesDependedOn  : {RPCSS, HTTP}\n"
                    output += "CanPauseAndContinue : False\n"
                    output += "CanShutdown         : False\n"
                    output += "CanStop             : True\n"
                    output += "ServiceType         : Win32OwnProcess\n"
