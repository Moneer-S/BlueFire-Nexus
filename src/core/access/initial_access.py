    ...
    def _handle_email(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if email_type == "spear":
                ...
                result["details"]["research_depth"] = (
                    "Deep - social media, professional profiles, recent activities"
                )
                ...
                result["details"]["personalization"] = (
                    "Very high - deep research on executive's communications style"
                )
                result["details"]["research_depth"] = (
                    "Extensive - public appearances, speeches, writing style analysis"
                )
            ...
            result["details"]["email_content"] = {
                "from_display": (
                    ...
                    if email_type == "mass"
                        ...
                        "Mark Johnson, IT Security"
                        if email_type == "spear"
                        else "David Williams, CFO"
                    )
                ),
                "from_email": (
                    "security@legitimate-looking-domain.com"
                    if email_type == "mass"
                    else (
                        "mark.johnson@similar-company-domain.com"
                        if email_type == "spear"
                        else "d.williams@executive-domain.com"
                    )
                ),
                ...
                "pretext": (
                    ...
                    if email_type == "mass"
                        ...
                        if email_type == "spear"
                        else "Financial approval required"
                    )
                ),
            }
            ...
            result["details"]["delivery"] = {
                ...
                "sending_pattern": (
                    ...
                    if email_type != "mass"
                    else "Bulk send with rate limiting"
                ),
                "timing": "Tuesday morning, business hours",
                "spoofing_technique": (
                    "Display name spoofing"
                    if email_type == "mass"
                    else "Domain lookalike"
                    if email_type == "spear"
                    else "Modified mail headers"
                ),
            }
            ...
            if attachment:
                attachment_types = {
                    "spear": [
                        {
                            "name": "Q2_Financial_Review.xlsm",
                            "type": "Excel Macro",
                            "payload": "VBA macro dropper",
                        },
                        {
                            "name": "Employee_Survey_Results.docm",
                            "type": "Word Macro",
                            "payload": "VBA macro with PowerShell execution",
                        },
                        {
                            "name": "Project_Timeline.pdf",
                            "type": "PDF Exploit",
                            "payload": "JavaScript code execution via Adobe Reader vulnerability",
                        },
                    ],
                    "whaling": [
                        {
                            "name": "Acquisition_Agreement_Draft.docm",
                            "type": "Word Macro",
                            "payload": "VBA macro with direct shellcode execution",
                        },
                        {
                            "name": "Board_Presentation.ppam",
                            "type": "PowerPoint Macro",
                            "payload": "VBA macro launching hidden PowerShell",
                        },
                        {
                            "name": "Executive_Compensation.xlsm",
                            "type": "Excel Macro",
                            "payload": "Excel 4.0 macro (XLM) execution",
                        },
                    ],
                    "mass": [
                        {
                            ...
                            "type": "Executable",
                            "payload": "Trojanized security tool",
                        },
                        {
                            "name": "Company_Directory.pdf",
                            "type": "PDF Exploit",
                            "payload": "Malicious JavaScript execution",
                        },
                        {
                            "name": "HR_Form.doc",
                            "type": "Word Macro",
                            "payload": "VBA macro downloader",
                        },
                    ],
                }

                result["details"]["attachment"] = random.choice(attachment_types[email_type])
                result["details"]["attachment"]["obfuscation"] = (
                    "Password protected archive"
                    if email_type != "mass"
                    else "Double extension (.pdf.exe)"
                )
                result["details"]["attachment"]["detection_bypass"] = (
                    "Custom packer"
                    if email_type == "whaling"
                    else (
                        "Encrypted payload sections"
                        if email_type == "spear"
                        else "Simple XOR encoding"
                    )
                )
            ...
            if link:
                link_types = {
                    "spear": [
                        {
                            "url": "https://sharepoint-secure-docs.com/document/review",
                            "type": "Credential Harvester",
                            "disguise": "SharePoint login portal",
                        },
                        {
                            "url": "https://company-portal-login.com/auth",
                            "type": "Credential Harvester",
                            "disguise": "Company VPN portal",
                        },
                        {
                            "url": "https://secure-document-view.org/statement",
                            "type": "Exploit Kit Landing",
                            "disguise": "Document viewer application",
                        },
                    ],
                    "whaling": [
                        {
                            "url": "https://secure-executive-portal.com/login",
                            "type": "Credential Harvester",
                            "disguise": "Executive dashboard login",
                        },
                        {
                            "url": "https://board-documents-secure.com/meeting",
                            "type": "Exploit Kit Landing",
                            "disguise": "Board document portal",
                        },
                        {
                            "url": "https://secure-signature-system.com",
                            "type": "Credential Harvester",
                            "disguise": "Digital signature system",
                        },
                    ],
                    "mass": [
                        {
                            "url": "https://password-reset-portal.com",
                            "type": "Credential Harvester",
                            "disguise": "Password reset form",
                        },
                        {
                            "url": "https://it-security-verify.com",
                            "type": "Drive-by Download",
                            "disguise": "Security verification page",
                        },
                        {
                            "url": "https://employee-portal-access.com",
                            "type": "Credential Harvester",
                            "disguise": "Employee portal",
                        },
                    ],
                }

                result["details"]["link"] = random.choice(link_types[email_type])
                result["details"]["link"]["obfuscation"] = (
                    "URL shortener"
                    if email_type == "mass"
                    else (
                        "Typosquatting domain"
                        if email_type == "spear"
                        else "Subdomain of legitimate site"
                    )
                )
                result["details"]["link"]["hosting"] = (
                    "Compromised WordPress site"
                    if email_type == "mass"
                    else (
                        "Custom phishing infrastructure"
                        if email_type == "whaling"
                        else "Cloud-hosted landing page"
                    )
                )
            ...
            compromise_rate = (
                0.3 if email_type == "spear" else 0.4 if email_type == "whaling" else 0.05
            )
            ...
            result["details"]["mitre_technique_name"] = (
                ...
                if attachment
                else "Phishing: Spearphishing Link"
            )
