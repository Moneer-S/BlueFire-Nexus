...
class DefenseEvasion:
    ...
    def __init__(self, execution_module: "Execution"):
        ...
        if not execution_module:
            ...
            def _execute_command_wrapper(
                command: str, method: str = "direct", capture: bool = True
            ) -> Dict[str, Any]:
                exec_data = {
                    "execute": {
                        "command": {"cmd": command, "method": method, "capture_output": capture}
                    }
                }
                try:
                    ...
                    logger.error(
                        f"Failed to execute command '{command}' via Execution module: {e}",
                        exc_info=True,
                    )
        ...
        self.supported_techniques = list(self.general_techniques.keys()) + list(
            self.os_supported_techniques
        )
        logger.info(
            f"Supported defense evasion techniques on {self.os_type}: {self.supported_techniques}"
        )
        ...
        if action == "hide":
            if self.os_handler and "file_hide" in self.os_handler.handler_map:
                ...
                return {
                    ...
                    "reason": f"File hiding not supported on {self.os_type}",
                }
        elif action == "timestomp":
        ...
        try:
            if mode == "mimic":
                ...
                try:
                    ...
                    self.logger.info(
                        f"Using specific timestamps: AT={access_time_ts}, MT={modify_time_ts}"
                    )
            ...
            if access_time_ts is not None and modify_time_ts is not None:
                ...
                if (
                    abs(new_stat.st_atime - access_time_ts) < 1
                    and abs(new_stat.st_mtime - modify_time_ts) < 1
                ):
                    status = "success"
    ...
    def _handle_argument_spoofing(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not original_command or not spoofed_command:
            return {
                "status": "error",
                "reason": "Missing 'command' or 'spoofed_command' in details.",
                "mitre_technique_id": mitre_id,
                "mitre_technique_name": mitre_name,
            }
        ...
        try:
            ...
            exec_result = self._execute_command(
                original_command, method=execution_method, capture=True
            )

            result_details["execution_result"] = (
                exec_result  # Include actual execution details for debugging/info
            )

            # Report success/failure based on the *actual* execution
            status = exec_result.get("status", "failure")
            if status != "success":
                result_details["reason"] = exec_result.get(
    ...
    def _handle_pid_spoofing(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.os_handler and "pid_spoofing" in self.os_handler.handler_map:
            ...
            return {
                "status": "not_implemented",
                "reason": f"PID Spoofing not supported on {self.os_type}",
    ...
    def _handle_process_evasion(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "status": "not_implemented",
            "reason": "Process Evasion handler not implemented",
    ...
    def _handle_network_evasion(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "status": "not_implemented",
            "reason": "Network Evasion handler not implemented",
        ...
        def execute(self, data: Dict[str, Any]) -> Dict[str, Any]:
            ...
            return {
                "results": {
                    "command_execution": {
                        "status": "success",
                        "details": {"return_code": 0, "stdout": "Mock success", "stderr": ""},
                    }
                }
            }
    ...
    try:
        ...
        hide_request = {
            "evade": {
                ...
                "details": {"target_file": test_file_path, "action": "hide"},
            }
        }
        ...
        if platform.system() != "Windows":  # On Unix, check rename
            ...
            print(
                f"Checking if file exists: {hidden_test_path} -> {os.path.exists(hidden_test_path)}"
            )
        ...
        target_for_stomp = (
            hidden_test_path
            if platform.system() != "Windows" and os.path.exists(hidden_test_path)
            else test_file_path
        )
        stomp_mimic_request = {
            "evade": {
