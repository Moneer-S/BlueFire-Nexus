...
class LinuxExecution:
    ...
    def __init__(self):
        self.config = {"execution_timeout": 120, "default_shell": "bash"}  # Default config values
        self.handler_map = {
    ...
    def execute(self, exec_type: str, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if handler:
            ...
            return {
                "status": "failure",
                "type": exec_type,
                "reason": f"Unsupported execution type '{exec_type}' for Linux/Unix",
            }
    ...
    def _run_command(
        ...
        if use_shell:
            ...
            try:
                ...
                logger.warning(
                    f"Could not shlex.split command '{command}': {e}. Using basic split."
                )
    ...
    def _handle_command_execution(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        use_system_shell = data.get(
            "use_shell_api", True if method == "bash" else False
        ...
        if method == "disk":
            ...
            try:
                ...
                try:
                    ...
                    os.chmod(
                        temp_file_path,
                        current_st.st_mode | stat.S_IEXEC | stat.S_IXGRP | stat.S_IXOTH,
                    )
