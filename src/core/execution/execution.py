...
class Execution:
    ...
    def execute(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.os_handler:
            return {
                "status": "failure",
                "reason": f"Execution handler not available for OS: {self.os_type}",
            }
        ...
        if "command" in execution_requests:
            exec_type = "command"
            details = (
                execution_requests["command"]
                if isinstance(execution_requests["command"], dict)
                else {}
            )
            ...
            details = (
                ...
                if isinstance(execution_requests["payload"], dict)
                else {}
            )
            if not details.get("content_b64") or not details.get("method"):
                errors.append(
                    "Payload execution requested but 'content_b64' or 'method' is missing."
                )
                details = None  # Prevent execution
        ...
        if exec_type and details:
            try:
                ...
                if exec_result.get("status") != "success":
                    errors.append(
                        exec_result.get("reason", f"{exec_type.capitalize()} execution failed")
                    )
            ...
            errors.append(
                "No valid 'command' or 'payload' execution request found in 'execute' data."
            )
    ...
    def execute_command(
        self, command: str, capture_output: bool = True, method: str = "direct"
    ) -> Dict[str, Any]:
        ...
        command_details = {"cmd": command, "method": method, "capture_output": capture_output}
...
if __name__ == "__main__":
    ...
    if platform.system() == "Windows":
        cmd_request_ps = {
            "execute": {
                "command": {
                    "cmd": "Get-Process | Select-Object -First 3 Name, Id",
                    "method": "powershell",
                }
            }
        }
    ...
    if platform.system() != "Windows":
        cmd_request_bash = {
            "execute": {
                "command": {"cmd": "echo 'Hello from Bash' && ls -la | head -n 5", "method": "bash"}
            }
        }
    ...
    cmd_request_fail = {
        "execute": {"command": {"cmd": "nonexistent_command_12345", "method": "direct"}}
    }
    ...
    if platform.system() == "Windows":
        cmd_request_ps_b64 = {
            "execute": {
                "command": {
                    "cmd": "Write-Host 'Hello from Base64 PS!'; $env:USERNAME",
                    "method": "powershell",
                    "obfuscation": "base64",
                }
            }
    ...
    if platform.system() == "Windows":
        ...
        cmd_request_caret = {
            "execute": {
                "command": {
                    "cmd": "echo Hello & echo World | findstr World",
                    "method": "cmd",
                    "obfuscation": "caret",
                }
            }
    ...
    if platform.system() != "Windows":
        cmd_request_concat = {
            "execute": {
                "command": {"cmd": "whoami", "method": "bash", "obfuscation": "string_concat"}
            }
        }
