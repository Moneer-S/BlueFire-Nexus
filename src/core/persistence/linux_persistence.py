...
class LinuxPersistence:
    ...
    def establish(self, technique: str, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if handler:
            ...
            try:
                ...
                logger.error(
                    f"Error executing Linux persistence technique '{technique}': {e}", exc_info=True
                )
            ...
            return {
                "status": "failure",
                "technique": technique,
                "reason": "Unsupported technique for Linux/macOS",
            }
    ...
    def _handle_profile_script(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        command = details.get("command")
        comment = details.get(
            "comment", "BlueFire Persistence Marker"
        )  # Default comment for idempotency
        ...
        if target_scripts_names:
            ...
            for name in target_scripts_names:
                ...
                if script_path.is_file():
                    ...
                    logger.warning(
                        f"Specified target script '{name}' not found at {script_path}. Skipping."
                    )
        else:
        ...
        if not scripts_to_modify:
            return {
                "status": "failure",
                "technique": "profile_script",
                "reason": "No suitable profile scripts found to modify.",
            }
        ...
        for script_path in scripts_to_modify:
            ...
            try:
                ...
                if check_marker in script_content:
                    reason = (
                        f"Marker '{comment}' already found in {script_path}. Skipping modification."
                    )
                    logger.info(reason)
        ...
        return {
            ...
            "reason": (
                f"{len(failed_files)} script(s) failed modification." if failed_files else None
            ),
        }
