    ...
    def establish(self, technique: str, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if handler:
            ...
            try:
                ...
                logger.error(
                    f"Error executing Windows persistence technique '{technique}': {e}",
                    exc_info=True,
                )
            ...
            return {
                "status": "failure",
                "technique": technique,
                "reason": "Unsupported technique for Windows",
            }
    ...
    def _handle_scheduled_task(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not command:
            return {
                ...
                "reason": "Missing 'command' in details.",
            }
        ...
        schtasks_command = [
            ...
            task_name,
            "/tr",
            f"{command}",  # Ensure command quoting is handled if needed
            "/sc",
            trigger,
            "/rl",
            "HIGHEST",
    ...
    def _handle_registry_run_key(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if hive_str not in valid_hives:
            return {
                "status": "failure",
                "technique": "registry_run_key",
                "reason": f"Invalid hive: {hive_str}. Use HKCU or HKLM.",
            }
        if key_type not in valid_key_types:
            return {
                "status": "failure",
                "technique": "registry_run_key",
                "reason": f"Invalid key_type: {key_type}. Use Run or RunOnce.",
            }
        ...
        logger.info(
            rf"Attempting to set registry {key_type} key: {hive_str}\{base_key_path}\{value_name}"
        )
        ...
        try:
            ...
            key_handle = win32api.RegOpenKeyEx(
                hive, base_key_path, 0, win32con.KEY_SET_VALUE | win32con.KEY_WOW64_64KEY
            )  # Use 64-bit view
    ...
    def _get_startup_folder(self, scope: str = "user") -> Optional[str]:
        ...
        try:
            if scope.lower() == "user":
                ...
                logger.error(
                    f"Invalid scope specified for startup folder: {scope}. Use 'user' or 'system'."
                )
                return None
    ...
    def _handle_startup_folder(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not command and not payload_content:
            return {
                ...
                "reason": "Missing 'command' or 'payload_content' in details.",
            }
        ...
        if not startup_dir:
            return {
                "status": "failure",
                "technique": "startup_folder",
                "reason": f"Could not determine Startup folder path for scope '{scope}'.",
            }
