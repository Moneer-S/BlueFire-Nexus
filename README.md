...
- **Provider-specific AI adapter for Gemini.** Anthropic's Messages-API adapter ships and is auto-registered for the `anthropic` canonical name (with `claude` alias normalised to it). Gemini's GenerateContent API still needs its own request/response adapter before it can leave the keyless stub. Other vendors (`openai`, `grok`, `ollama`, `llama.cpp`, `lm-studio`) work via the OpenAI-compatible HTTP backend — see [docs/reports/ai_layer.md](docs/reports/ai_layer.md).
