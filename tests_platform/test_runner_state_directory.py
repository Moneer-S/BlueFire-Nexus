...
def fresh_home(
    ...
    # Exercise both default path layouts on each POSIX host. The bootstrap is
    # a storage-only fake; no platform-specific native process is started.
    current_platform = runner_bootstrap.current_platform
    monkeypatch.setattr(
        runner_bootstrap,
        "current_platform",
        lambda value=None: current_platform(value or request.param),
    )
    return home
...
def test_existing_state_ancestor_permissions_and_contents_are_preserved(fresh_home: Path) -> None:
    local = managed_product_root().parent.parent
...
def test_xdg_state_directory_follows_the_platform_storage_contract(
    ...
    _manager(root).bootstrap(allowed_profile_ids=("sandbox-execute.v1",))
    assert root.is_dir()
    if runner_bootstrap.current_platform() == "macos":
        assert root == fresh_home / "Library" / "Application Support" / "BlueFire Nexus"
        assert not base.exists()
    else:
        assert root == base / "bluefire-nexus"
...
def test_linked_default_ancestor_is_refused_without_target_writes(
    ...
    with pytest.raises(RunnerLifecycleError, match="unavailable or unsafe"):
        _manager(root).bootstrap(allowed_profile_ids=("sandbox-execute.v1",))
    assert not (outside / root.parent.name).exists()
...
def test_parent_replacement_during_creation_is_detected(
    ...
    replaced = False
    retained = fresh_home / "retained-state-parent"

    def replace_parent(path: Any, mode: int = 0o777, *, dir_fd: int | None = None) -> None:
        nonlocal replaced
        if path == root.parent.name and dir_fd is not None and not replaced:
            replaced = True
    ...
    assert replaced
    assert not (outside / root.parent.name).exists()
    assert not (retained / root.parent.name / root.name).exists()
...
def test_creation_parent_owned_by_another_user_is_refused(
    ...
    assert not managed_product_root().parent.parent.exists()
