...
mod collection;
use collection::{ArchiveTarAction, BundleFormat, CollectionMethodAction, CollectionStageAction};
...
mod tests {
    ...
    #[test]
    fn atomic_gzip_has_fixed_linux_contract_and_rejects_unreviewed_inputs() {
        let action = AtomicGzipAction;
        assert_eq!(action.descriptor().platforms, &[Platform::Linux]);
        assert!(action.descriptor().process_effect);
        assert!(!action.descriptor().network_effect);
        assert!(action.descriptor().cleanup_receipt);
        let valid = json!({"input": "fixtures/transformed.jsonl", "expected_sha256": "a".repeat(64), "stage_variant": "primary"});
        assert!(action.prepare(valid.clone()).is_ok());
        for (field, value) in [
            ("input", json!("/etc/passwd")),
            ("stage_variant", json!("elsewhere")),
            ("expected_sha256", json!("bad")),
            ("executable", json!("/bin/sh")),
            ("arguments", json!(["-c", "caller"])),
        ] {
    ...
    #[test]
    fn atomic_gzip_failure_preserves_execution_phase_and_timeout_truth() {
        use crate::atomic_gzip::GzipError;
        assert_eq!(
            atomic_gzip_failure(GzipError::from("missing utility")).status,
            TaskStatus::ControlBlocked
        );
        assert_eq!(
            atomic_gzip_failure(GzipError::failed("child exited unsuccessfully")).status,
            TaskStatus::Failed
        );
        assert_eq!(
            atomic_gzip_failure(GzipError::timed_out("deadline elapsed")).status,
            TaskStatus::TimedOut
        );
    }
