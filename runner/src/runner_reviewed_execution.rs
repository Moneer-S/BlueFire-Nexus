...
fn validate_identity(operation: &ReviewedOperationIdentity) -> Result<(), ActionFailure> {
        ...
        .is_some_and(|digest| !is_sha256_digest(digest))
    {
        return Err(blocked(
            ...
            "reviewed execution binding digest is invalid",
        ));
    }
    Ok(())
}
...
pub(super) fn validate_profile(profile: &RunnerProfile) -> Result<(), ActionFailure> {
    let Some(authority) = &profile.reviewed_execution else {
        return Ok(());
    };
    if authority.schema_version != SCHEMA
        || !is_sha256_digest(&authority.authorization_digest)
        || authority.operations.is_empty()
        || authority.operations.len() > MAX_OPERATIONS
    {
        return Err(blocked(
            "invalid_reviewed_execution",
            "reviewed execution schema, digest or bounds are invalid",
        ));
    }
    if authority
        .operations
        ...
        .any(|pair| pair[0] >= pair[1])
    {
        return Err(blocked(
            "invalid_reviewed_execution",
            "reviewed operations must be unique and canonically ordered",
        ));
    }
    ...
    for operation in &authority.operations {
        ...
        let native = profile.action_bindings.iter().find(|binding| {
            binding.logical_behavior_id == operation.behavior_id
                && binding.logical_action_id == operation.action_id
        });
        ...
        let digest = match (native, provider) {
            ...
            _ => {
                return Err(blocked(
                    "invalid_reviewed_execution",
                    "reviewed operation has no unique registered execution binding",
                ))
            }
        };
        if digest != operation.execution_binding_digest {
            return Err(blocked(
                "invalid_reviewed_execution",
                "reviewed operation binding differs from the profile",
            ));
        }
        if let Some(digest) = digest {
            used_bindings.insert(digest);
        }
    }
    if profile
        .action_bindings
        .iter()
        .any(|binding| !used_bindings.contains(&binding_hash(binding)))
        || profile
            .provider_bindings
            .iter()
            .any(|binding| !used_bindings.contains(&binding_hash(binding)))
    {
        return Err(blocked(
            "invalid_reviewed_execution",
            "profile includes an unreviewed execution binding",
        ));
    }
    if profile
        .allowed_actions
        ...
        || profile
            ...
            .iter()
            .any(|action| required.contains(action))
    {
        return Err(blocked(
            "invalid_reviewed_execution",
            "allowed actions must exactly cover reviewed operations without blocked actions",
        ));
    }
    Ok(())
}
...
pub(super) fn validate_manifest(
    ...
    let (authority, selected) = match (&profile.reviewed_execution, &manifest.reviewed_operation) {
        ...
        _ => {
            return Err(blocked(
                ...
                "reviewed operation is required only with reviewed execution authority",
            ))
        }
    };
    let digest = match (&manifest.execution_binding, &manifest.provider_binding) {
        ...
        _ => {
            return Err(blocked(
                "reviewed_operation_blocked",
                "manifest has competing execution bindings",
            ))
        }
    };
    ...
    if selected.authorization_digest != authority.authorization_digest
        || selected.step_id != identity.step_id
        || selected.behavior_id != identity.behavior_id
        || selected.action_id != identity.action_id
        || selected.execution_binding_digest != identity.execution_binding_digest
        || !authority.operations.contains(&identity)
    {
        return Err(blocked(
            "reviewed_operation_blocked",
            "manifest operation is outside reviewed execution authority",
        ));
    }
    Ok(())
}
